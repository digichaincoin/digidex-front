...
export function useDerivedZapInfo(): {
  ...
  const parsedAmount = tryParseAmount(
    halfTypedValue === "NaN" ? "0" : halfTypedValue,
    inputCurrency ?? undefined
  );

  const bestZapOne = useTradeExactIn(
    parsedAmount,
    outputCurrencies[0] ?? undefined
  );
  const bestZapTwo = useTradeExactIn(
    parsedAmount,
    outputCurrencies[1] ?? undefined
  );
  const zap = useMemo(
    () =>
      mergeBestZaps(
  ...
  if (balanceIn && amountIn && JSBI.lessThan(balanceIn.raw, amountIn)) {
    inputError = `Insufficient ${zap.currencyIn.currency.getSymbol(
      chainId
    )} balance`;
  }
...
export function useDefaultCurrencies() {
  ...
  useEffect(() => {
    const outputCurrencies = {
      currency1: "ETH",
      currency2: contracts.digichain[chainId],
    };
