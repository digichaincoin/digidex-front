...
export const fetchBillsAllowance = async (
  chainId: number,
  account: string,
  bills: BillsConfig[]
) => {
  ...
  return bills.reduce(
    (acc, bill, index) => ({
      ...acc,
      [bill.index]: new BigNumber(allowances[index]).toString(),
    }),
    {}
  );
};
...
export const fetchUserBalances = async (
  ...
  const tokenBalances = bills.reduce(
    (acc, bill, index) => ({
      ...acc,
      [bill.index]: new BigNumber(tokenBalancesRaw[index]).toString(),
    }),
    {}
  );
...
export const fetchUserOwnedBills = async (
  ...
  const pendingRewardsCall = await multicall(
    chainId,
    billAbi,
