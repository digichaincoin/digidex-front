...
const cleanBillsData = (
  ...
  const data = chunkedBills.map((chunk, index) => {
    ...
    const earnTokenPrice = tokenPrices?.find(
      (token) =>
        token.address[chainId] === billConfig.earnToken.address[chainId]
    )?.price;
    const [
      trueBillPrice,
      currentDebt,
    ...
    const [controlVariable, vestingTerm, minimumPrice, maxPayout, maxDebt] =
      terms;
    const priceUsd = getBalanceNumber(trueBillPrice) * lpPrice;
    const discount = ((earnTokenPrice - priceUsd) / earnTokenPrice) * 100;
    const formatedPrice = priceUsd
      ? getFirstNonZeroDigits(priceUsd)
      : undefined;
    return {
      ...billConfig,
