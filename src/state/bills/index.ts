  ...
  reducers: {
    ...
    setBillsPublicData: (state, action) => {
      ...
      state.data = state.data.map((bill) => {
        const liveBillData = liveBillsData.find(
          (entry) => entry.index === bill.index
        );
    ...
    setBillsUserData: (state, action) => {
      ...
      state.data = state.data.map((bill) => {
        const userBillData = userData.find(
          (entry) => entry.index === bill.index
        );
    ...
    setUserOwnedBillsData: (state, action) => {
      const userData = action.payload;
      state.data = state.data.map((bill) => {
        const userOwnedBillsData = userData.find(
          (entry) => entry.index === bill.index
        );
        return {
          ...bill,
          userOwnedBillsData: userOwnedBillsData?.userOwnedBills,
        };
      });
    },
    setUserOwnedBillsNftData: (state, action) => {
      const userData = action.payload;
      state.data = state.data.map((bill) => {
        const userOwnedBillsNftData = userData.find(
          (entry) => entry.index === bill.index
        );
        return {
          ...bill,
          userOwnedBillsNftData: userOwnedBillsNftData?.userOwnedBillsNfts,
        };
      });
    },
    updateBillsUserData: (state, action) => {
      ...
      state.data[i] = {
        ...state.data[i],
        userData: { ...state.data[i].userData, [field]: value },
      };
    },
    updateBillsUserNftData: (state, action) => {
      ...
      state.data[i] = {
        ...
        userOwnedBillsNftData: {
          ...state.data[i].userOwnedBillsNftData,
          ...value,
        },
      };
    },
...
export const setInitialBillsDataAsync =
  (chainId: number) => async (dispatch) => {
    try {
      ...
      const filterBillsByChainId = initialBillState.filter(
        (bill) => bill.contractAddress?.[chainId] !== ""
      );
...
export const fetchBillsUserDataAsync =
  ...
  async (dispatch, getState) => {
    try {
      ...
      const stakingTokenBalances = await fetchUserBalances(
        chainId,
        account,
        bills
      );
  ...
  async (dispatch, getState) => {
    try {
      ...
      const mapUserOwnedBills = bills.map((bill) =>
        userOwnedBills.filter(
          (b) => b.address === bill.contractAddress[chainId]
        )
      );
      ...
      const userBillNftData = await fetchUserOwnedBillNftData(
        ownedBillsData,
        chainId
      );
      const ownedBillsWithNftData = mapUserOwnedBills.map((bs, index) => {
        return {
          ...
          userOwnedBillsNfts: [
            ...bs.map((b) => {
              return userBillNftData.find(
                (nftB) => parseInt(nftB.id) === parseInt(b.id)
              )?.data;
            }),
          ],
        };
...
export const updateUserNftData =
  ...
  async (dispatch) => {
    const fetchedBillNftData = await getNewBillNftData(
      billNftId,
      transactionHash,
      chainId
    );
