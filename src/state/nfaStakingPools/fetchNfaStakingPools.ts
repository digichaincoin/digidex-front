import nonFungibleApesAbi from "config/abi/nonFungibleDigis.json";
...
export const fetchPoolsTotalStatking = async (
  ...
  const nfaStakingPoolTotalStaked = await multicall(
    chainId,
    nonFungibleApesAbi,
    calls
  );
  ...
  const mappedValues = nfaStakingPoolsConfig.map((pool) => {
    ...
    const rewardToken = tokenPrices
      ? tokenPrices.find(
          (token) =>
            pool?.rewardToken &&
            token?.address[CHAIN_ID] === pool?.rewardToken.address[CHAIN_ID]
        )
      : pool.rewardToken;
    const totalStaked = totalStakingList.find(
      (totalStake) => totalStake.sousId === pool.sousId
    )?.totalStaked;
    // Calculate apr
