...
import {
  ...
  fetchPoolTokenStatsAndApr,
} from "./fetchNfaStakingPools";
import {
  fetchPoolsAllowance,
  fetchUserBalances,
  ...
  reducers: {
    ...
    setNfaStakingPoolsPublicData: (state, action) => {
      ...
      state.data = state.data.map((nfaStakingPool) => {
        const liveNfaStakingPoolData = liveNfaStakingPoolsData.find(
          (entry) => entry.sousId === nfaStakingPool.sousId
        );
...
export const setInitialNfaStakingDataAsync = () => async (dispatch) => {
  try {
    const initialNfaStakingState: NfaStakingPool[] =
      await fetchNfaStakingConfig();
...
export const fetchNfaStakingPoolsPublicDataAsync =
  ...
  async (dispatch, getState) => {
    ...
    const tokenStatsAndAprs = await fetchPoolTokenStatsAndApr(
      tokenPrices,
      totalStakings,
      nfaStakingPools
    );
    const liveData = await Promise.all(
      nfaStakingPools.map(async (nfaStakingPool) => {
        ...
        const tokenStatsAndApr = tokenStatsAndAprs.find(
          (entry) => entry.sousId === nfaStakingPool.sousId
        );
        return {
          ...blockLimit,
...
export const fetchNfaStakingPoolsUserDataAsync =
  ...
  async (dispatch, getState) => {
    const nfaStakingPools = getState().nfaStakingPools.data;
    const allowances = await fetchPoolsAllowance(
      chainId,
      account,
      nfaStakingPools
    );
    ...
    const stakedBalances = await fetchUserStakeBalances(
      chainId,
      account,
      nfaStakingPools
    );
    const pendingRewards = await fetchUserPendingRewards(
      chainId,
      account,
      nfaStakingPools
    );
    const stakedNfas = await fetchUserStakedNfas(
      chainId,
      account,
      nfaStakingPools
    );
...
export const updateNfaStakingUserBalance =
  ...
  async (dispatch, getState) => {
    const nfaStakingPools = getState().nfaStakingPools.data;
    const tokenBalances = await fetchUserBalances(
      chainId,
      account,
      nfaStakingPools
    );
...
export const updateUserNfaStakingPendingReward =
  ...
  async (dispatch, getState) => {
    const nfaStakingPools = getState().nfaStakingPools.data;
    const pendingRewards = await await fetchUserStakedNfas(
      chainId,
      account,
      nfaStakingPools
    );
