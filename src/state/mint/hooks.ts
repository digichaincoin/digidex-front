...
export function useMintActionHandlers(noLiquidity: boolean | undefined): {
  ...
  const onUserInput = useCallback(
    (field: Field, typedValue: string) => {
      dispatch(
        typeInput({ field, typedValue, noLiquidity: noLiquidity === true })
      );
    },
...
export function useDerivedMintInfo(
  ...
  if (
    ...
    error = `${t("Insufficient")} ${currencies[Field.CURRENCY_A]?.getSymbol(
      chainId
    )} ${t("balance")}`;
  }
  ...
  if (
    ...
    error = `${t("Insufficient")} ${currencies[Field.CURRENCY_B]?.getSymbol(
      chainId
    )} ${t("balance")}`;
  }
