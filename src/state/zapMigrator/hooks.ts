...
export function useZapMigratorState(): AppState["zapMigrator"] {
  return useSelector<AppState, AppState["zapMigrator"]>(
    (state) => state.zapMigrator
  );
}
  ...
  const amountsMinAdd = {
    [Field.CURRENCY_A]: parsedAmounts?.CURRENCY_A
      ? calculateSlippageAmount(
          new TokenAmount(
            parsedAmounts?.CURRENCY_A.token,
            amountsMinRemove[Field.CURRENCY_A]
          ),
          allowedSlippage
        )[0]
      : "0",
    [Field.CURRENCY_B]: parsedAmounts?.CURRENCY_B
      ? calculateSlippageAmount(
          new TokenAmount(
            parsedAmounts?.CURRENCY_B.token,
            amountsMinRemove[Field.CURRENCY_B]
          ),
          allowedSlippage
        )[0]
      : "0",
...
export const useMigratorBalances = (): {
  ...
  const callResult = useSingleCallResult(
    migratorBalanceCheckerContract,
    "getBalance",
    [account],
    options
  );
  ...
  const loLpAddresses = result
    ? result.pBalances.flatMap((b) => b.balances.map((item) => item.lp))
    : [];
  // List of Token addresses
  const loTokenAddresses = result
    ? result.pBalances.flatMap((b) =>
        b.balances.flatMap((item) => [item.token0, item.token1])
      )
    : [];

  const lpCallResults = useMultipleContractSingleData(
    loLpAddresses,
    PAIR_INTERFACE,
    "getReserves"
  );
  ...
  const tokenSymbolResults = useMultipleContractSingleData(
    loTokenAddresses,
    PAIR_INTERFACE,
    "symbol"
  );
  const tokenDecimalResults = useMultipleContractSingleData(
    loTokenAddresses,
    PAIR_INTERFACE,
    "decimals"
  );
  ...
  const balanceData = useMemo(() => {
    return result
      ? result.pBalances.flatMap((b, i) => {
          ...
          return b.balances.map(
            ([pid, lp, token0, token1, total, wallet, staked]) => {
              return {
                smartRouter: chef,
                ...
                totalSupply: getFullDisplayBalance(
                  new BigNumber(
                    sortedTotalSupply.find((item) => item.address === lp)?.value
                  )
                ),
                token0: {
                  address: token0,
                  symbol: sortedSymbols.find((item) => item.address === token0)
                    ?.value,
                  decimals: sortedDecimals.find(
                    (item) => item.address === token0
                  )?.value,
                  reserves: sortedReserves
                    .find((item) => item.address === lp)
                    ?.value?.[0].toString(),
                },
                token1: {
                  address: token1,
                  symbol: sortedSymbols.find((item) => item.address === token1)
                    ?.value,
                  decimals: sortedDecimals.find(
                ...
                walletBalance: getFullDisplayBalance(
                  new BigNumber(wallet.toString())
                ),
                stakedBalance: getFullDisplayBalance(
                  new BigNumber(staked.toString())
                ),
                totalBalance: getFullDisplayBalance(
