  ...
  reducers: {
    ...
    setLoadVaultData: (state, action) => {
      ...
      state.data = state.data.map((vault) => {
        const liveVaultData = liveVaultsData.find(
          (entry) => entry.id === vault.id
        );
...
export const setInitialVaultDataAsync =
  (chainId: number) => async (dispatch) => {
    try {
      const initialVaultState: Vault[] = await fetchVaultConfig();
      const filteredVaults = initialVaultState?.filter((vault) =>
...
export const fetchVaultUserDataAsync =
  ...
  async (dispatch, getState) => {
    try {
      ...
      const userVaultAllowances = await fetchVaultUserAllowances(
        account,
        chainId,
        vaults
      );
      const userVaultTokenBalances = await fetchVaultUserTokenBalances(
        account,
        chainId,
        vaults
      );
      const userVaultBalances = await fetchVaultUserStakedAndPendingBalances(
        account,
        chainId,
        vaults
      );
...
export const updateVaultUserAllowance =
  ...
  async (dispatch, getState) => {
    ...
    dispatch(
      updateVaultsUserData({ id, field: "allowance", value: allowances[index] })
    );
  };
...
export const updateVaultUserStakedBalance =
  ...
  async (dispatch, getState) => {
    const vaults = getState().vaults.data;
    const stakedBalances = await fetchVaultUserStakedAndPendingBalances(
      account,
      chainId,
      vaults
    );
    ...
    dispatch(
      updateVaultsUserData({
        id,
        field: "stakedBalance",
        value: stakedBalances.stakedBalances[index],
      })
    );
  };
