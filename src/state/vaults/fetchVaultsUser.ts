...
import vaultApeV1ABI from "config/abi/vaultDigiV1.json";
import vaultApeV2ABI from "config/abi/vaultDigiV2.json";
import multicall from "utils/multicall";
import {
  getVaultDigiAddressV1,
  getVaultDigiAddressV2,
...
export const fetchVaultUserAllowances = async (
  ...
  const vaultApeAddressV1 = getVaultDigiAddressV1(chainId);
  const vaultApeAddressV2 = getVaultDigiAddressV2(chainId);
  ...
  const calls = filteredVaults.map((vault) => {
    return {
      ...
      params: [
        account,
        vault.version === "V1" ? vaultApeAddressV1 : vaultApeAddressV2,
      ],
    };
  });
...
export const fetchVaultUserStakedAndPendingBalances = async (
  ...
  const parsePendingBalances = rawStakedBalances.map((stakedBalance, index) => {
    return filteredVaults[index].version === "V1"
      ? "0"
      : new BigNumber(stakedBalance[1]._hex).toJSON();
  });
  return {
    stakedBalances: parsedStakedBalances,
    pendingRewards: parsePendingBalances,
  };
};
