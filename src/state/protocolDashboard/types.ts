...
export interface OverviewProtocolMetricsInterface {
  description: "Digichain Holders" | "Market Cap" | "Digichain Burned" | "POL";
  amount: number;
  history: { amount: number; time: number }[];
}

export interface OverviewDigichainDistributionInterface {
  total: number;
...
export interface ProtocolDashboardState {
  ...
  overviewDigichainDistribution: OverviewDigichainDistributionInterface[];
