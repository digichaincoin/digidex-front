...
export const iazosSlice = createSlice({
  ...
  reducers: {
        ...
        state.iazoData = state.iazoData
          ? state.iazoData.map((iazo) => {
              const liveIazoData = state.iazoData.find(
                (entry) =>
                  entry.iazoContractAddress === iazo.iazoContractAddress
              );
    ...
    updateIazoWeb3Data: (state, action) => {
      ...
      const index = state.iazoData.findIndex(
        (p) => p.iazoContractAddress === contractAddress
      );
...
export const fetchIazos = (chainId: number) => async (dispatch) => {
  try {
    ...
    iazos?.map(async (iazo) => {
      const isRegestered = await isRegisteredIazoCheck(
        chainId,
        iazo.iazoContractAddress
      );
      dispatch(
        updateIazoWeb3Data({
          value: isRegestered,
      ...
      if (isRegestered?.isRegistered) {
        const iazoTokenDetails = await fetchIazoTokenDetails(
          chainId,
          iazo.baseToken.address,
          iazo.iazoToken.address
        );
        dispatch(
          updateIazoWeb3Data({
        ...
        const iazoStatusInfo = await fetchIazoStatusInfo(
          chainId,
          iazo.iazoContractAddress
        );
        dispatch(
          updateIazoWeb3Data({
            value: iazoStatusInfo,
            contractAddress: iazo.iazoContractAddress,
          })
        );
      }
...
export const fetchIazo =
  (chainId: number, address: string) => async (dispatch) => {
    try {
      dispatch(iazosFetchStart());
