...
const fetchIazoDefaultSettings = async (chainId: number) => {
  ...
  const [fetchIazoDefaultSetting, fetchIazoDelaySetting] = await multicall(
    chainId,
    iazoSettingsAbi,
    [
...
export const fetchIazoTokenDetails = async (
  chainId: number,
...
export const fetchIazoStatusInfo = async (chainId: number, address: string) => {
  ...
  const [feeInfo, status, iazoState, iazoInfo] = await multicall(
    chainId,
    iazoAbi,
    calls
  );
  ...
  return {
    ...
    iazoState: iazoStates[iazoState.toString()],
  };
};

export const isRegisteredIazoCheck = async (
  chainId: number,
  address: string
) => {
  ...
  const resp = await multicall(chainId, iazoExposerABI, [
    {
      address: iazoExposerAddress,
      name: "IAZOIsRegistered",
      params: [address],
    },
  ]);
