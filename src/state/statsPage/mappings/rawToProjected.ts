...
export function rawToProjected({
  userStats,
  digichainPrice,
  analytics,
}: ApiResponse) {
  ...
  userStats.forEach((chainInfo) => {
    if (chainInfo.farms.length > 0) {
      chainInfo.farms.forEach((farm) => {
        const farmApy =
          "earnedBalance" in farm
            ? parseFloat(farm.digichainApr).toFixed(2)
        ...
        const projectedProduct = projectedProducts.find(
          (product) => product.type === "digichain farms"
        );
    ...
    if (chainInfo.jungleFarms.length > 0) {
      chainInfo.jungleFarms.forEach((farm) => {
        ...
        const projectedProduct = projectedProducts.find(
          (product) => product.type === "jungle farms"
        );
    ...
    if (chainInfo.maximizers.length > 0) {
      chainInfo.maximizers.forEach((vault) => {
        ...
        const projectedProduct = projectedProducts.find(
          (product) => product.type === "maximizers"
        );
