...
export function rawToPortfolio({ userStats, digichainPrice }: ApiResponse) {
  ...
  userStats.forEach((chainInfo) => {
    if (chainInfo.farms.length > 0) {
      chainInfo.farms.forEach((farm) => {
        ...
        const positionEarnings = positionData.secondaryRewardBalance
          ? positionData.rewardBalance * digichainPrice +
            positionData.secondaryRewardValue
          : positionData.rewardBalance * digichainPrice;
    ...
    if (chainInfo.pools.length > 0) {
      chainInfo.pools.forEach((pool) => {
        ...
        const positionEarnings =
          positionData.rewardBalance * positionData.rewardToken.price;
      ...
      chainInfo.lending.markets.forEach((lending) => {
        ...
        if (product) {
          ...
          products.push({
            ...
            totalValue: positionData.value,
            totalEarnings: chainInfo.lending.earnedBalance * digichainPrice,
          });
      });
    }
    ...
    if (chainInfo.maximizers.length > 0) {
      chainInfo.maximizers.forEach((vault) => {
        ...
        const positionEarnings = positionData.rewardBalance * digichainPrice;
        ...
        if (product) {
          ...
          products.push({
            ...
            totalValue: positionData.value,
            totalEarnings: positionData.rewardBalance * digichainPrice,
          });
      });
    }
