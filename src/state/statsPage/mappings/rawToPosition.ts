...
export const farmToPosition = (farm: Farm | DualFarm): Position => {
  const isNFTY2 =
    farm.stakedToken.pairData?.token1.address.toLowerCase() === NFTY2_ADDRESS;
  ...
  const tokens = {
    ...
    token2: wrappedToNative(
      isNFTY2 ? "NFTY2" : farm.stakedToken.pairData?.token1.symbol
    ),
    token3: isDualFarm
      ? wrappedToNative(farm.earnTokens.token1.symbol)
      : "DIGICHAIN",
    token4: isDualEarnings
      ? wrappedToNative(isNFTY2 ? "NFTY2" : farm.earnTokens.token2.symbol)
      : null,
  };
  ...
  return {
    ...
    rewardBalance: isDualFarm
      ? +farm.earnedBalances.balance1
      : +farm.earnedBalance,
    secondaryRewardBalance: isDualEarnings
      ? +farm.earnedBalances.balance2
      : null,
    secondaryRewardValue: isDualEarnings
      ? +farm.earnedBalances.balance2 * farm.earnTokens.token2.price
      : null,
    apr: isDualFarm
      ? +farm.earnApr.toFixed(2)
      : +(+farm.digichainApr).toFixed(2),
    tokens,
    isLp: true,
  };
};
...
export const maximizerToPosition = (vault: Vault): Position => {
  ...
  else tokens = { token1: "DIGICHAIN", token2: "DIGICHAIN" };
