  ...
  reducers: {
    ...
    setDualFarmUserData: (state, action) => {
      ...
      state.data = state.data.map((dualFarm) => {
        const userDualFarmData = userData.find(
          (entry) => entry.pid === dualFarm.pid
        );
...
// Actions
export const {
  setInitialDualFarmData,
  setDualFarmsPublicData,
  setDualFarmUserData,
  updateDualFarmUserData,
...
export const fetchDualFarmUserDataAsync =
  ...
  async (dispatch, getState) => {
    try {
      const dualFarms = getState().dualFarms.data;
      const userFarmAllowances = await fetchDualFarmUserAllowances(
        chainId,
        account,
        dualFarms
      );
      const userFarmTokenBalances = await fetchDualFarmUserTokenBalances(
        chainId,
        account,
        dualFarms
      );
      const userStakedBalances = await fetchDualFarmUserStakedBalances(
        chainId,
        account,
        dualFarms
      );
      const miniChefEarnings = await fetchDualMiniChefEarnings(
        chainId,
        account,
        dualFarms
      );
      const rewarderEarnings = await fetchDualFarmRewarderEarnings(
        chainId,
        account,
        dualFarms
      );
      const arrayOfUserDataObjects = dualFarms.map((dualFarm) => {
        return {
          pid: dualFarm.pid,
          allowance: userFarmAllowances.find(
            (item) => item.pid === dualFarm.pid
          ).value,
          tokenBalance: userFarmTokenBalances.find(
            (item) => item.pid === dualFarm.pid
          ).value,
          stakedBalance: userStakedBalances.find(
            (item) => item.pid === dualFarm.pid
          ).value,
          miniChefEarnings: miniChefEarnings.find(
            (item) => item.pid === dualFarm.pid
          ).value,
          rewarderEarnings: rewarderEarnings.find(
            (item) => item.pid === dualFarm.pid
          ).value,
        };
      });
...
export const updateDualFarmUserAllowances =
  ...
  async (dispatch, getState) => {
    const dualFarms = getState().dualFarms.data;
    const allowances = await fetchDualFarmUserAllowances(
      chainId,
      account,
      dualFarms
    );
    ...
    dispatch(
      updateDualFarmUserData({
        pid,
        field: "allowance",
        value: allowances[pidIndex].value,
      })
    );
  };
...
export const updateDualFarmUserTokenBalances =
  ...
  async (dispatch, getState) => {
    const dualFarms = getState().dualFarms.data;
    const tokenBalances = await fetchDualFarmUserTokenBalances(
      chainId,
      account,
      dualFarms
    );
    ...
    dispatch(
      updateDualFarmUserData({
        pid,
        field: "tokenBalance",
        value: tokenBalances[pidIndex].value,
      })
    );
  };
...
export const updateDualFarmUserStakedBalances =
  ...
  async (dispatch, getState) => {
    const dualFarms = getState().dualFarms.data;
    const stakedBalances = await fetchDualFarmUserStakedBalances(
      chainId,
      account,
      dualFarms
    );
    ...
    dispatch(
      updateDualFarmUserData({
        pid,
        field: "stakedBalance",
        value: stakedBalances[pidIndex].value,
      })
    );
  };
...
export const updateDualFarmUserEarnings =
  ...
  async (dispatch, getState) => {
    const dualFarms = getState().dualFarms.data;
    const pendingRewards = await fetchDualMiniChefEarnings(
      chainId,
      account,
      dualFarms
    );
    ...
    dispatch(
      updateDualFarmUserData({
        pid,
        field: "miniChefEarnings",
        value: pendingRewards[pidIndex].value,
      })
    );
  };
...
export const updateDualFarmRewarderEarnings =
  ...
  async (dispatch, getState) => {
    ...
    dispatch(
      updateDualFarmUserData({
        pid,
        field: "rewarderEarnings",
        value: rewarderEarnings[pidIndex].value,
      })
    );
  };
