...
export const usePollDualFarms = () => {
  ...
  const digichainPrice = usePriceDigichainBusd().toString();
  ...
  useEffect(() => {
    ...
    fetchFarms();
  }, [dispatch, chainId, tokenPrices, digichainPrice, farmLpAprs]);
};
...
export const useDualFarms = (account): DualFarm[] => {
  ...
  useEffect(() => {
    if (
      account &&
      (chainId === ChainId.MATIC || chainId === ChainId.MATIC_TESTNET)
    ) {
...
export const useFarmFromPid = (pid): DualFarm => {
  const farm = useSelector((state: State) =>
    state.dualFarms.data.find((f) => f.pid === pid)
  );
  return farm;
};
...
export const useFarmUser = (pid) => {
  ...
  return {
    ...
    earnings: farm?.userData
      ? new BigNumber(farm.userData.rewarderEarnings)
      : new BigNumber(0),
  };
};
