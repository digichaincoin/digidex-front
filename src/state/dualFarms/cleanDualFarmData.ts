...
const cleanDualFarmData = (
  ...
  const data = chunkedFarms.map((chunk, index) => {
    const dualFarmConfig = dualFarms?.find(
      (farm) => farm.pid === farmIds[index]
    );
    const quoteToken = tokenPrices?.find(
      (token) =>
        token.address[chainId] ===
        dualFarmConfig.stakeTokens.token0.address[chainId]
    );
    const token1 = tokenPrices?.find(
      (token) =>
        token.address[chainId] ===
        dualFarmConfig.stakeTokens.token1.address[chainId]
    );
    const miniChefRewarderToken = tokenPrices?.find(
      (token) =>
        token.address[chainId] ===
        dualFarmConfig.rewardTokens.token0.address[chainId]
    );
    const rewarderToken = tokenPrices?.find(
      (token) =>
        token.address[chainId] ===
        dualFarmConfig.rewardTokens.token1.address[chainId]
    );
    const lpApr =
      farmLpAprs?.lpAprs?.find((lp) => lp.pid === dualFarmConfig.pid)?.lpApr *
      100;
    ...
    if (
      ...
      rewarderPoolWeight = rewarderAllocPoint.div(
