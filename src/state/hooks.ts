...
import { Toast, toastTypes } from "@digidexfinance/uikit";
...
export const useNetworkChainIdFromUrl = (): boolean => {
  const chainIdFromUrl = useSelector(
    (state: State) => state.network.data.chainIdFromUrl
  );
...
export const useProfile = () => {
  const { isInitialized, isLoading, data }: ProfileState = useSelector(
    (state: State) => state.profile
  );
  return {
    profile: data,
    hasProfile: isInitialized && data !== null,
    isInitialized,
    isLoading,
  };
};
...
export const useHomepageServiceStats = (): ServiceData[] => {
  const homepageServiceStats = useSelector(
    (state: State) => state.stats.HomepageServiceStats
  );
  return homepageServiceStats;
};

export const useFetchHomepageTokenStats = (
  isFetching: boolean,
  category: string
) => {
  const dispatch = useAppDispatch();
...
export const useHomepageTokenStats = (): HomepageTokenStats[] => {
  const homepageTokenStats = useSelector(
    (state: State) => state.stats.HomepageTokenStats
  );
...
export const useHomepageLaunchCalendar = (): LaunchCalendarCard[] => {
  const homepageLaunchCalendar = useSelector(
    (state: State) => state.stats.HomepageLaunchCalendar
  );
...
export const useAuctions = () => {
  const { isInitialized, isLoading, data }: AuctionsState = useSelector(
    (state: State) => state.auctions
  );
...
export const useIazos = () => {
  const { isInitialized, isLoading, iazoData }: IazosState = useSelector(
    (state: State) => state.iazos
  );
  return { iazos: iazoData, isInitialized, isLoading };
};

export const useIazoSettings = () => {
  const { iazoDefaultSettings }: IazosState = useSelector(
    (state: State) => state.iazos
  );
...
export const useIazoFromAddress = (address): Iazo => {
  const iazo: Iazo = useSelector((state: State) =>
    state.iazos.iazoData.find((i) => i.iazoContractAddress === address)
  );
  return iazo;
};
...
export const useNfas = () => {
  const { isInitialized, isLoading, data }: NfaState = useSelector(
    (state: State) => state.nfas
  );
...
export const useLpTokenPrices = () => {
  const { isInitialized, isLoading, data }: LpTokenPricesState = useSelector(
    (state: State) => state.lpTokenPrices
  );
...
export const usePendingUsd = () => {
  const { isInitialized, isLoading, data }: StatsState = useSelector(
    (state: State) => state.stats
  );
  return {
    pending: data?.pendingRewardUsd || 0,
    hasStats: isInitialized && data !== null,
    isInitialized,
    isLoading,
  };
};
...
export const useGetPoolStats = (pid) => {
  ...
  const { isInitialized, isLoading, data }: StatsOverallState = useSelector(
    (state: State) => state.statsOverall
  );
  if (isInitialized) {
