...
export const fetchPoolStats = (pools: Pool[], curBlock): any[] => {
  const stakedPools = pools.filter(
    (pool) => parseInt(pool?.userData?.stakedBalance?.toString()) > 0
  );
  const mappedPoolStats = stakedPools.map((pool) => {
    ...
    const pendingRewardUsd = pool.rewardToken
      ? pendingReward * pool?.rewardToken?.price
      : 0;
    const dollarsEarnedPerDay = (stakedTvl * (pool?.apr / 100)) / 365;
    const tokensEarnedPerDay = pool.rewardToken
      ? dollarsEarnedPerDay / pool?.rewardToken?.price
      : 0;
...
export function computeStats(
  ...
  const stats = {
    ...
    digichainsInWallet: digichainsInWallet.toString(),
    pendingRewardUsd: 0,
  ...
  stats.pools.forEach((pool) => {
    ...
    stats.pendingRewardDigichain += pool.pendingReward;
    stats.dollarsEarnedPerDay += pool.dollarsEarnedPerDay;
    stats.digichainsEarnedPerDay += pool.tokensEarnedPerDay;
  ...
  stats.farms.forEach((farm) => {
    ...
    stats.pendingRewardDigichain += farm.pendingReward;
    stats.dollarsEarnedPerDay += farm.dollarsEarnedPerDay;
    stats.digichainsEarnedPerDay += farm.tokensEarnedPerDay;
  ...
  stats.digichainsEarnedPerWeek = stats.digichainsEarnedPerDay * 7;
  stats.digichainsEarnedPerMonth = stats.digichainsEarnedPerDay * 30;
  stats.digichainsEarnedPerYear = stats.digichainsEarnedPerDay * 365;
  return stats;
}
...
export function getStatsForFarms(farms, farmsInfo) {
  ...
  farms.map(async (farm) => {
    ...
    const userInfo = {
      ...farm.userData,
      amount: parseInt(farm.userData.stakedBalance, 10),
    };
    ...
    if (farmInfo && (userInfo.amount !== 0 || pendingReward !== 0)) {
      ...
      const tokensEarnedPerDay =
        dollarsEarnedPerDay / farmInfo.rewardTokenPrice;
      const currFarm = {
        pid: farm.pid,
