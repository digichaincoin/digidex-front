...
import nfbABI from "config/abi/nonFungibleDigichains.json";
import {
  getNonFungibleDigisAddress,
  getNonFungibleDigichainsAddress,
...
const getProfile = async (
  ...
  const nfbAddress = getNonFungibleDigichainsAddress(chainId);
  ...
  try {
    ...
    if (nfasOwned !== "0" && !nfasOwned.eq(0)) {
      ...
      rarestNft = ownedNfts
        ? orderBy(ownedNfts, ["attributes.rarityOverallRank"])[0]
        : null;
