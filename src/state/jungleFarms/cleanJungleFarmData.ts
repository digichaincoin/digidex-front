...
const cleanJungleFarmData = (
  ...
  const data = chunkedFarms.map((chunk, index) => {
    const farmConfig = jungleFarmsConfig.find(
      (farm) => farm.jungleId === farmIds[index]
    );
...
const fetchJungleFarmTokenStatsAndApr = (
  ...
  const rewardToken = tokenPrices
    ? tokenPrices.find(
        (token) =>
          farm?.rewardToken &&
          token?.address[chainId] === farm?.rewardToken.address[chainId]
      )
    : farm.rewardToken;
  const stakingToken = tokenPrices
    ? tokenPrices.find(
        (token) =>
          token?.address[chainId] === farm?.stakingToken.address[chainId]
      )
    : farm.stakingToken;
  // Calculate apr
