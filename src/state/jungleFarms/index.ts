  ...
  reducers: {
    ...
    setJungleFarmsPublicData: (state, action) => {
      ...
      state.data = state.data.map((farm) => {
        const liveFarmData = liveJungleFarmsData.find(
          (entry) => entry.jungleId === farm.jungleId
        );
    ...
    setJungleFarmsUserData: (state, action) => {
      ...
      state.data = state.data.map((farm) => {
        const userFarmData = userData.find(
          (entry) => entry.jungleId === farm.jungleId
        );
...
// Actions
export const {
  setInitialJungleFarmData,
  setJungleFarmsPublicData,
  setJungleFarmsUserData,
  updateJungleFarmsUserData,
...
export const fetchJungleFarmsUserDataAsync =
  ...
  async (dispatch, getState) => {
    try {
      const jungleFarms = getState().jungleFarms.data;
      const allowances = await fetchJungleFarmsAllowance(
        chainId,
        account,
        jungleFarms
      );
