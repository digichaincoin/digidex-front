...
const getInputFractionAfterFee = (
  chainId: number,
  smartRouter: SmartRouter
) => {
  const baseFee = new Percent(
    JSBI.BigInt(SMART_LP_FEES[chainId][smartRouter]),
    JSBI.BigInt(10000)
  );
    ...
    : ONE_HUNDRED_PERCENT.subtract(
        trade.route.pairs.reduce<Fraction>(
          (currentFee: Fraction): Fraction =>
            currentFee.multiply(getInputFractionAfterFee(chainId, smartRouter)),
          ONE_HUNDRED_PERCENT
        )
      );
