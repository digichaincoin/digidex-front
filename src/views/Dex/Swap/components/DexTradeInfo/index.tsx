...
import { Flex, HelpIcon, Svg, Text, TooltipBubble } from "@digi.swap/uikit";
import { Trade } from "@digi.swap/sdk";
...
const DexTradeInfo: React.FC<{
  ...
  const route = trade?.route?.path?.map((val, i) => {
    return i < trade?.route?.path?.length - 1
      ? `${val.getSymbol(chainId)} > `
      : val.getSymbol(chainId);
  });
        ...
        <Flex>
          ...
          >
            <Text
              ...
              sx={{
                ...dexStyles.textWrap,
                lineHeight: isBonusRouter ? "10px" : "10px",
              }}
            >
              {isBonusRouter
                ? t("Bonus Router")
          ...
          >
            ...
            >
                    ...
                    body={
                      ...
                      >
                        {isBonusRouter
                          ...
                          : isSmartRouter
                          ? t(
                              "The router used to facilitate transactions through other DEXs if DigiDex cannot fulfill them."
                            )
                          : t(
                              "The native router that powers the majority of trades on the ApeSwap DEX."
                            )}
                      </Text>
                    }
                    placement="topRight"
              ...
              {isBonusRouter && (
                <>
                  <Flex
                    sx={{ justifyContent: "space-between", margin: "2px 0px" }}
                  >
                    <Text
                      color="primaryBright"
                      size="10px"
                    ...
                    >
                      ~{" "}
                      {(
                        bestRoute?.bonusRouter?.summary?.searchSummary
                          ?.expectedKickbackProfit * 0.3
                      ).toFixed(6)}{" "}
                      {CHAIN_PARAMS[chainId].nativeCurrency.symbol}{" "}
                      {`(~$${(
                        bestRoute?.bonusRouter?.summary?.searchSummary
                          ?.expectedUsdProfit * 0.3
                      ).toFixed(2)})`}
                    </Text>
                  </Flex>
                </>
