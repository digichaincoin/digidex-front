...
import { currencyEquals, Trade } from "@digi.swap/sdk";
import { ModalProps } from "@digidexfinance/uikit";
...
const ConfirmSwapModal: React.FC<ModalProps & ConfirmSwapModalProps> = ({
  ...
  } ${trade?.inputAmount?.currency?.getSymbol(chainId) ?? ""} for ${
        ...
        <TransactionErrorContent
          ...
          message={
            swapErrorMessage.includes("INSUFFICIENT_OUTPUT_AMOUNT")
              ? t(
                  "Slippage Error: Please check your slippage using the ⚙️ icon & try again!"
                )
              : swapErrorMessage
          }
        />
