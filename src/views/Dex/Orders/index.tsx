...
import { Flex, Text, useModal } from "@digi.swap/uikit";
import { Link } from "@digidexfinance/uikit";
import { computeLegacyPriceBreakdown } from "utils/prices";
import { useHistory } from "react-router-dom";
import { useTranslation } from "contexts/Localization";
import { CurrencyAmount, JSBI, Token, Trade } from "@digi.swap/sdk";
import {
...
const Orders: React.FC = () => {
  ...
  const [onPresentImportTokenWarningModal] = useModal(
    <ImportTokenWarningModal
      ...
      onCancel={() => history.push("/limit-orders")}
    />
  );
  ...
  const { INPUT, OUTPUT, independentField, typedValue, recipient } =
    useSwapState();
  ...
  const [inputCurrency, outputCurrency] = [
    useCurrency(INPUT?.currencyId),
    useCurrency(OUTPUT?.currencyId),
  ];

  const { wrapType, execute: onWrap } = useWrapCallback(
  ...
  const outputMinAmount = useMemo(() => {
    return (
      Number(realPriceValue) * Number(formattedAmounts[Field.INPUT])
    ).toString();
  ...
  const orderMarketStatus = useMemo(() => {
    ...
    if (marketOutput && outputMinAmount) {
      return (
        ((Number(outputMinAmount) - Number(marketOutput)) * 100) /
        Number(marketOutput)
      );
    }
    return 0;
