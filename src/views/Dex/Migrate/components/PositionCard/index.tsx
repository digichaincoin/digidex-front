...
import { SmartRouter, Token } from "@digi.swap/sdk";
import {
  Text,
  Flex,
  CardProps,
...
export default function FullPositionCard({
  ...
  const poolTokenPercentage =
    parseFloat(userBalance) / parseFloat(totalPoolTokens?.raw.toString());
  ...
  const tokenObj0 = new Token(
    chainId,
    token0.address,
    token0.decimals,
    token0.symbol
  );
  const tokenObj1 = new Token(
    chainId,
    token1.address,
    token1.decimals,
    token1.symbol
  ...
  const matchingFarm = farms.find(
    (farm) =>
      (farm.tokenAddresses[chainId].toLowerCase() ===
        token0.address.toLowerCase() ||
        farm.tokenAddresses[chainId].toLowerCase() ===
          token1.address.toLowerCase()) &&
      (farm.quoteTokenAdresses[chainId].toLowerCase() ===
        token0.address.toLowerCase() ||
        farm.quoteTokenAdresses[chainId].toLowerCase() ===
          token1.address.toLowerCase())
  );
      ...
      <Flex sx={{ ...styles.innerContainer }}>
            ...
            >
              ...
              <Text size="12px" weight={700}>
                {!token0 || !token1 ? (
                  ...
                  `${wrappedToNative(token0.symbol)}-${wrappedToNative(
                    token1.symbol
                  )}`
                )}
              </Text>
              <Text size="10px" weight={400} sx={{ lineHeight: "5px" }}>
                {currencyPrice
                  ? `$${(currencyPrice * parseFloat(userBalance)).toFixed(2)}`
                  : "-"}
              </Text>
            </Flex>
          </Flex>
        ...
        <Flex sx={{ alignItems: "center" }}>
          <Button
            ...
            to={
              inWallet
                ? `migrate/${token0.address}/${token1.address}`
                : `unstake/${token0.address}/${token1.address}`
            }
        ...
        {showMore && (
          ...
          >
            ...
            <Flex sx={{ width: "100%", flexDirection: "column", mb: "10px" }}>
              <Flex
                sx={{ justifyContent: "space-between", margin: "2.5px 0px" }}
              >
                  ...
                  <Flex sx={{ flexDirection: "column" }}>
                    <Text size="12px" weight={400} sx={{ lineHeight: "20px" }}>
                      ...
                      <span sx={{ fontWeight: 700 }}>
                        {" "}
                        {matchingFarm?.apy || ""}% APY{" "}
                      </span>
                      with the{" "}
                      <span sx={{ fontWeight: 700 }}>
                        {wrappedToNative(token0.symbol)}-
                        {wrappedToNative(token1.symbol)} farm at ApeSwap
                      </span>
                    </Text>
                    <UiLink
                      href={`https://digidex.finance/digichain-farms?pid=${matchingFarm.pid}`}
                      target="_blank"
                    >
