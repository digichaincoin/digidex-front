...
const MigrateProgress: React.FC<MigrateProcessBarInterface> = ({
  activeLineMargin,
  children,
}) => {
  const { activeIndex, migrateLpStatus, setActiveIndexCallback } =
    useMigrateAll();
    ...
    <Flex sx={{ flexDirection: "column" }}>
      <Flex sx={{ alignItems: "center" }}>
        {MIGRATION_STEPS.map(({ title, description }, i) => {
          const isIndexComplete =
            isComplete.filter((loFlag) => !loFlag[i]).length === 0;
          return (
            <>
              <Flex
                key={title}
              ...
              >
                ...
                <Flex sx={{ alignItems: "center", justifyContent: "center" }}>
                  <Text
                    ...
                    weight={700}
                    mr="5px"
                    color={activeIndex >= i ? "primaryBright" : "text"}
                  >
                    {title}
                  </Text>
                  <Flex sx={{ mt: "2.5px" }}>
      ...
      >
        <Flex
          sx={{
            ...
            width: "10px",
            left: `${
              (activeIndex / MIGRATION_STEPS.length) * 100 +
                activeIndex +
                activeLineMargin || 15
            }%`,
            position: "absolute",
