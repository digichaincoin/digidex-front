...
import { Button, Flex, Svg, Text, TooltipBubble } from "@digi.swap/uikit";
...
const Migrate: React.FC<{
  ...
  const listView = migrateList?.map((migrate) => {
    ...
    const status = migrateLpStatus.find(
      (status) => status.lpAddress === lpAddress
    );
    const matchedApeLps = apeswapWalletLps?.find(
      ({ pair }) =>
        pair.token0.address === token0.address &&
        pair.token1.address === token1.address
    );
    return {
      ...
      stakeLp: true,
      title: `${wrappedToNative(token0.symbol)} - ${wrappedToNative(
        token1.symbol
      )}`,
      noEarnToken: true,
      id: lpAddress,
      cardContent: (
        <>
          <ListViewContent
            title={t("LP To Maigrate")}
            value={walletBalance}
            ml={20}
          />
          <ListViewContent
            title={t("Ape LP")}
            value={matchedApeLps?.balance?.toSignificant(6) || "0"}
            ml={20}
          />
          <ListViewContent
            title={t("Status")}
