...
import { Button, Flex, Text } from "@digi.swap/uikit";
...
const ApproveStake: React.FC<{
  apeswapWalletLps: { pair: Pair; balance: TokenAmount }[];
}> = ({ apeswapWalletLps }) => {
  const { chainId, account } = useActiveWeb3React();
  const farms = useFarms(account);
  const { t } = useTranslation();
  const { migrateLpStatus, migrateMaximizers, setMigrateMaximizersCallback } =
    useMigrateAll();
  ...
  const filteredLpsForStake = filteredLps?.filter(
    ({ pair }) =>
      migrateLpStatus?.find(
        (status) =>
          status.lpAddress.toLowerCase() ===
          pair.liquidityToken.address.toLowerCase()
    ...
    return {
      ...
      cardContent: (
        <>
          <ListViewContent
            title={t("Wallet")}
            value={balance?.toSignificant(6) || "0"}
            ml={20}
          />
