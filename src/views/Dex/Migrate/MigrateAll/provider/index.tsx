...
import { Pair, TokenAmount } from "@digi.swap/sdk";
...
const MigrateContext = createContext<MigrateContextData>(
  {} as MigrateContextData
);
...
export function MigrateProvider({ children }: MigrateProviderProps) {
  ...
  const migrateWalletBalances = valid
    ? migrateLpBalances.filter((bal) => parseFloat(bal.walletBalance) > 0.0)
    : [];
  const migrateStakedBalances = valid
    ? migrateLpBalances.filter((bal) => parseFloat(bal.stakedBalance) > 0.0)
    : [];
  ...
  const filteredLpsForStake = apeswapLpBalances?.filter((lp) =>
    farms?.find(
      (farm) =>
        lp.pair.liquidityToken.address.toLowerCase() ===
        farm.lpAddresses[chainId].toLowerCase()
    )
  );
  console.log(farms, vaults);
  const farmAndVaultUserDataLoaded =
    farms?.[0]?.userData !== undefined && vaults?.[0]?.userData !== undefined;
  ...
  const setActiveIndexCallback = useCallback(
    (activeIndex: number) => setActiveIndex(activeIndex),
    []
  );
  ...
  const handleUpdateMigrateLp = useCallback(
    (lpAddress, type, status, statusText) => {
      ...
      const lpToUpdateIndex = lpStatus.findIndex(
        (migrateLp) => migrateLp.lpAddress === lpAddress
      );
      const lpToUpdate = {
