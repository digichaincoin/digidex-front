...
import { Pair, TokenAmount, ZAP_ADDRESS } from "@digi.swap/sdk";
...
export const setMigrateLpStatus = async (
  ...
  const getMigrateLpStatus = async () => {
    const calls = migrateLps?.map((migrateLp) => {
      return {
        address: migrateLp.lpAddress,
        name: "allowance",
        params: [account, ZAP_ADDRESS[chainId]],
      };
    });
    ...
    return migrateLps?.map((migrateLp, i) => {
      return {
        ...
        status: {
          unstake:
            parseFloat(migrateLp.stakedBalance) > 0
              ? MigrateStatus.INCOMPLETE
              : MigrateStatus.COMPLETE,
          approveMigrate: new BigNumber(rawLpAllowances[i]).gt(0)
            ? MigrateStatus.COMPLETE
            : MigrateStatus.INCOMPLETE,
          migrate:
            parseFloat(migrateLp.walletBalance) > 0 ||
            parseFloat(migrateLp.stakedBalance) > 0
              ? MigrateStatus.INCOMPLETE
              : MigrateStatus.COMPLETE,
...
export const activeIndexHelper = (migrateLpStatus: MigrateLpStatus[]) => {
  const isComplete = migrateLpStatus?.map((item) =>
    Object.entries(item.status).map(
      (each) => each[1] === MigrateStatus.COMPLETE
    )
  );
