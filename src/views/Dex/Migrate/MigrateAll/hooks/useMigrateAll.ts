...
import { JSBI, SMART_ROUTER_ADDRESS } from "@digi.swap/sdk";
    ...
    (migrateLps: MigrateResult[]) => {
      migrateLps.map(async (migrateLp) => {
        ...
        const poolTokenPercentage =
          parseFloat(walletBalance) / parseFloat(totalSupply);
        ...
        const [token0MinIn, token1MinIn] = [
          JSBI.divide(
            JSBI.multiply(token0MinOut, JSBI.BigInt(10000 - allowedSlippage)),
            JSBI.BigInt(10000)
          ),
          JSBI.divide(
            JSBI.multiply(token1MinOut, JSBI.BigInt(10000 - allowedSlippage)),
            JSBI.BigInt(10000)
          ),
        ];
