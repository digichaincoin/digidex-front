...
import { useMasterChefAddress, useVaultDigiAddressV2 } from "hooks/useAddress";
...
const useStakeApproveAll = () => {
  ...
  const vaultAddress = useVaultDigiAddressV2();
    ...
    (apeswapWalletLps: { pair: Pair; balance: TokenAmount }[]) => {
      apeswapWalletLps.map(async ({ pair }) => {
        ...
        handleUpdateMigrateLp(
          ...
          `Pending ${
            migrateMaximizers && matchedVault ? "Maximizer" : "Farm"
          } Approval`
        );
        lpContract
          .approve(
