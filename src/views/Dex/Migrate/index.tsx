...
import { Text, Flex, Spinner } from "@digi.swap/uikit";
...
export default function Migrate() {
  ...
  const filterResultsByFarms = valid
    ...
    : [];
  const walletBalances = valid
    ? filterResultsByFarms.filter((bal) => parseFloat(bal.walletBalance) > 0.0)
    : [];
  const stakedBalances = valid
    ? filterResultsByFarms.filter((bal) => parseFloat(bal.stakedBalance) > 0.0)
    : [];
    ...
    <Flex sx={{ ...dexStyles.pageContainer }}>
      <Flex sx={{ flexDirection: "column" }}>
        <Flex sx={{ ...dexStyles.dexContainer }}>
          ...
          <LiquiditySubNav />
          <Flex
            sx={{ flexDirection: "column", maxWidth: "100%", width: "420px" }}
          >
            ...
            {(loading || !valid) && account ? (
              ...
              walletBalances?.length > 0 && stakedBalances?.length > 0 ? (
                ...
                >
                  ...
                  <Text
                    size="14px"
                    sx={{ margin: "10px 0px 5px 0px", opacity: ".5" }}
                  >
