...
import { JSBI, Percent, TokenAmount } from "@digi.swap/sdk";
...
function MigrateLiquidity({
  ...
  const { pair, parsedAmounts, error } = useDerivedZapMigratorInfo(
    currencyA ?? undefined,
    currencyB ?? undefined
  );
  const { valid, results } = useMigratorBalances();
  const stakedBalances = valid
    ? results.filter((bal) => parseFloat(bal.stakedBalance) > 0.0)
    : [];
  ...
  const { onUnstake } = useMigrateUnstake(
    lpToUnstake?.chefAddress,
    lpToUnstake?.pid
  );
  ...
  const handleUnstake = useCallback(
    (amount: string) => {
      ...
      onUnstake(amount)
        .then((resp) => {
          library
            .waitForTransaction(resp.transactionHash)
            .then((finishedTx) => {
              setPendingTx(false);
              toastSuccess(t("Withdraw Successful"), {
                text: t("View Transaction"),
                url: getEtherscanLink(
                  finishedTx.transactionHash,
                  "transaction",
                  chainId
                ),
              });
              history.push({
                pathname: `/migrate/${lpToUnstake.token0.address}/${lpToUnstake.token1.address}`,
              });
            });
        })
        .catch((e) => {
  ...
  const [approval, approveCallback] = useApproveCallback(
    amountToApprove,
    lpToUnstake?.chefAddress || undefined
  );
      ...
      <Flex sx={{ flexDirection: "column" }}>
        <Flex sx={{ ...dexStyles.dexContainer }}>
          ...
          <DexPanel
            ...
            panelText={`${t("Unstake From ")}${
              SMART_ROUTER_FULL_NAME[lpToUnstake?.smartRouter] || ""
            }:`}
            currency={currencyA}
