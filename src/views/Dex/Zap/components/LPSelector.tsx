...
import { Flex, Svg, Text, useModal } from "@digi.swap/uikit";
import { styles } from "../../components/TokenSelector/styles";
import { Spinner } from "theme-ui";
import { Pair, Token } from "@digi.swap/sdk";
...
const LPSelector: React.FC<{
  ...
  const [onPresentCurrencyModal] = useModal(
    <LPSearchModal onSelect={onSelect} />
  );
      ...
      {lpPair ? (
        <>
          ...
          <Text sx={styles.tokenText}>
            {wrappedToNative(lpPair.token0.getSymbol(chainId))}-
            {wrappedToNative(lpPair.token1.getSymbol(chainId))}
          </Text>
        </>
      ) : (
