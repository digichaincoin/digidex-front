...
const DexPanel: React.FC<DexPanelProps> = ({
  ...
  const selectedCurrencyBalance = useCurrencyBalance(
    account ?? undefined,
    isRemoveLiquidity
      ? lpPair?.liquidityToken ?? undefined
      : currency ?? undefined
  );
  const currencyBalance = userBalance
    ? userBalance?.toFixed(6)
    : selectedCurrencyBalance?.toSignificant(6) || "0";

  const { t } = useTranslation();

  const usdVal = useTokenPriceUsd(
    chainId,
    lpPair?.liquidityToken || currency,
    isRemoveLiquidity,
    smartRouter
  );
    ...
    <Flex sx={styles.dexPanelContainer}>
      <Flex sx={styles.panelTopContainer}>
        ...
        <NumericalInput
          ...
          disabled={
            (independentField && independentField !== fieldType && disabled) ||
            ordersDisabled
          }
          disabledText={
            independentField && independentField !== fieldType && disabled
            ...
            {(fieldType === Field.INPUT ||
              ...
              parseFloat(currencyBalance) > 0 && (
                <Flex
                  sx={styles.maxButton}
                  size="sm"
                  onClick={() => handleMaxInput(fieldType)}
                >
