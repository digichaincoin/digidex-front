...
import { Button, Flex, useModal } from "@digi.swap/uikit";
import { ETHER, ROUTER_ADDRESS } from "@digi.swap/sdk";
...
const AddLiquidityActions: React.FC<AddLiquidityActionsProps> = ({
  ...
  const [onPresentAddLiquidityModal] = useModal(
    <AddLiquidityModal
      title={
        noLiquidity ? t("You are creating a pool") : t("Confirm Add Liquidity")
      }
        ...
        <Flex sx={{ width: "100%" }}>
          <>
            {approvalA !== ApprovalState.APPROVED && (
              ...
              >
                {approvalA === ApprovalState.PENDING
                  ? `${t("Enabling")} ${currencies[Field.CURRENCY_A]?.getSymbol(
                      chainId
                    )}`
                  : `${t("Enable")} ${currencies[Field.CURRENCY_A]?.getSymbol(
                      chainId
                    )}`}
              </Button>
            )}
            {approvalB !== ApprovalState.APPROVED && (
              ...
              >
                {approvalB === ApprovalState.PENDING
                  ? `${t("Enabling")} ${currencies[Field.CURRENCY_B]?.getSymbol(
                      chainId
                    )}`
                  : `${t("Enable")} ${currencies[Field.CURRENCY_B]?.getSymbol(
                      chainId
                    )}`}
              </Button>
            )}
