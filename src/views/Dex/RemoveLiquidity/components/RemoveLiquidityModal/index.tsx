...
import { Modal } from "@digidexfinance/uikit";
...
const RemoveLiquidityModal: React.FC<RemoveLiquidityModalProps> = ({
  ...
  const pendingText = `${t("Removing")} ${
    parsedAmounts[Field.CURRENCY_A]?.toSignificant(6) ?? ""
  } ${currencyA?.getSymbol(chainId) ?? ""} and ${
      ...
      {attemptingTxn ? (
        ...
        <Flex sx={{ ...styles.modalWrapper }}>
          <>
            <Flex
              sx={{
                ...styles.confirmDisabledInputContainer,
                marginTop: "10px",
              }}
            >
              ...
              <Flex sx={{ alignItems: "center" }}>
                ...
                <Text size="14px" weight={700} ml="5px">
                  {`${currencyA?.getSymbol(chainId)} - ${currencyB?.getSymbol(
                    chainId
                  )}`}
                </Text>
              </Flex>
