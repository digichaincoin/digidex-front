...
import { Flex, Spinner, useMatchBreakpoints } from "@digidexfinance/uikit";
...
const Iazos: React.FC = () => {
  ...
  const currentIazos = registeredIazos?.filter(
    (iazo) =>
      ...
      currentTime <
        parseInt(iazo.timeInfo.startTime) +
          parseInt(iazo.timeInfo.activeTime) &&
      ...
      <PageWrapper>
        <Flex flexDirection="column">
          <Banner
            ...
            link="https://digichain.gitbook.io/digidex-finance/product-and-features/raise/self-serve-iao-ss-iao"
            maxWidth={856}
            listViewBreak
            margin="30px 0 0 0"
          />
          <LaunchPadWrapper>
            ...
            <IlosWrapper>
              <PresaleText>
                {(isInitialized || iazos) &&
                  `${renderIazos()?.length} Presales`}
              </PresaleText>
              {isInitialized || iazos ? (
                renderIazos()?.map((iazo) => {
                  return (
                    <Link
                      to={`/ss-iao/${iazo.iazoContractAddress}`}
                      key={iazo.iazoContractAddress}
                    >
