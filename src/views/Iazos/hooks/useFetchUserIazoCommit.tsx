...
const useFetchUserIazoCommit = (iazoAddress: string, dependency?: boolean) => {
  ...
  const [commited, setCommited] = useState<UserCommit>({
    deposited: null,
    tokensBought: null,
  });
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const buyers = await multicall(chainId, iazoAbi, [
          { address: iazoAddress, name: "BUYERS", params: [account] },
        ]);
        setCommited({
          deposited: buyers[0][0].toString(),
          tokensBought: buyers[0][1].toString(),
        });
      } catch (e) {
        console.error(e);
      }
