...
import { useMatchBreakpoints, Text } from "@digidexfinance/uikit";
...
const PairCreation: React.FC<PairCreationProps> = ({ onChange }) => {
  ...
  const accountFormated = ` ${account?.slice(0, 6)}...${account?.slice(
    account?.length - 4,
    account?.length
  )}`;
        ...
        onHandleERC20Details(tokenValue.toLowerCase())
          .then((resp) => {
            setSelectedToken({
              ...
              quoteToken: selectedToken.quoteToken,
            });
    ...
    <>
      <PairCreationWrapper>
        ...
        <TokenDropdown
          tokens={tokenList}
          onChange={(token) =>
            setSelectedToken({ ...selectedToken, quoteToken: token })
          }
        />
      </PairCreationWrapper>
      {tokenApproved && (
