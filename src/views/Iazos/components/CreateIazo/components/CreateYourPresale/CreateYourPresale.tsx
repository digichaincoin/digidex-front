...
const CreateIazo: React.FC<CreateIazoProps> = ({ settings }) => {
  ...
  const presaleStepsCompleted =
    stepper.pairCreated &&
    stepper.datesSelected &&
    stepper.presaleDetailsSet &&
    stepper.postsaleDetailsSet;
  ...
  const onInformation = useCallback((val) => {
    ...
    setStepper((prevState) => ({
      ...prevState,
      informationStepCompleted: val && true,
    }));
  }, []);
      ...
      {stepper.pairCreated && (
        <>
          <PresaleDetails
            ...
            pairTokenDetails={presaleData.pairCreation}
          />
