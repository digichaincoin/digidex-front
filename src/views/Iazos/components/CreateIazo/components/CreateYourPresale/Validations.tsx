...
export const postSaleValidation = (
  ...
  const minListPrice =
    presalePrice && parseFloat(presalePrice) - parseFloat(presalePrice) * 0.25;
  const maxListPrice =
    presalePrice && parseFloat(presalePrice) + parseFloat(presalePrice) * 0.5;
  if (!lockLiquidity) {
    validationList.push({
      error: t("Liquidity Lock Missing"),
      message: t("Please choose a liquidity lock time"),
    });
  }
  if (!liquidityPercent) {
    validationList.push({
...
const informationValidation = (
  ...
  const fileExtension =
    tokenLogo?.name?.split(".")[tokenLogo?.name?.split(".").length - 1];
  const fileSize = tokenLogo?.size;
  const acceptableFiles = ["jpg", "svg", "png"];
  if (!website) {
    validationList.push({
      error: t("Website URL Missing"),
      message: t("Please enter a website url"),
    });
  }
  if (!twitter) {
    validationList.push({
      error: t("Twitter URL Missing"),
      message: t("Please enter a twitter url"),
    });
  }
  if (!telegram) {
    validationList.push({
      error: t("Telegram URL Missing"),
      message: t("Please enter a telegram url"),
    });
  }
  if (!medium) {
    validationList.push({
      error: t("Medium URL Missing"),
      message: t("Please enter a medium url"),
    });
  }
  if (!tokenLogo) {
    validationList.push({
      error: t("Token Logo Missing"),
  ...
  if (!acceptableFiles.includes(fileExtension?.toLowerCase())) {
    validationList.push({
      error: t("Token Logo File Not Valid"),
      message: t("Please use a .jpg .png .svg instead"),
    });
  }
  if (fileSize > 1000000) {
    validationList.push({
  ...
  if (!isValidUrl(website)) {
    validationList.push({
      error: t("Website Is Not a Valid URL"),
      message: t("Please enter a valid website url"),
    });
  }
  if (!isValidUrl(twitter)) {
    validationList.push({
      error: t("Twitter Is Not a Valid URL"),
      message: t("Please enter a valid twitter url"),
    });
  }
  if (!isValidUrl(telegram)) {
    validationList.push({
      error: t("Telegram Is Not a Valid URL"),
      message: t("Please enter a valid telegram url"),
    });
  }
  if (!isValidUrl(medium)) {
    validationList.push({
      error: t("Medium Is Not a Valid URL"),
      message: t("Please enter a valid medium url"),
    });
  }
...
export const dateSelectionValidation = (
  ...
  if (activeTime < parseInt(minIazoLength)) {
    validationList.push({
      ...
      message: `${t("Please enter a longer IAZO time than the minimum")}: ${
        formattedMinTime.days
      }${t("days")}, ${formattedMinTime.hours}${t("hours")}, ${
        formattedMinTime.minutes
      } ${t("minutes")}`,
    });
  }
  if (activeTime > parseInt(maxIazoLength)) {
    validationList.push({
      ...
      message: `${t("Please enter a shorter IAZO time than the maximum")}: ${
        formattedMaxTime.days
      }${t("days")}, ${formattedMaxTime.hours}${t("hours")}, ${
        formattedMaxTime.minutes
      } ${t("minutes")}`,
    });
  }
  return validationList;
...
export const presaleValidation = (
  t,
  ...
  if (
    parseFloat(softcap) >
    parseFloat(tokensForSale) * parseFloat(pricePerToken)
  ) {
    validationList.push({
  ...
  if (!pricePerToken) {
    validationList.push({
      error: t("Price Per Token Missing"),
      message: t("Please enter a price per token"),
    });
  }
  if (!softcap) {
    validationList.push({
      error: t("Softcap Missing"),
      message: t("Please enter a softcap"),
    });
  }
  if (!limitPerUser) {
    validationList.push({
      error: t("Limit Per User Missing"),
      message: t("Please enter a limit per user"),
    });
  }
  if (!tokensForSale) {
    validationList.push({
...
const tokenAmountCheck = (
  ...
  const priceDifference = Math.abs(
    parseFloat(pricePerToken) / parseFloat(listingPrice)
  );
  const tokensForLiquidity =
    parseFloat(tokensForSale) * priceDifference * liquidityPercent;
  const totalTokens = tokensForLiquidity + parseFloat(tokensForSale);
  const formattedUserBalance = getBalanceNumber(
...
const Validations: React.FC<ValidationProps> = ({
  presaleData,
  settings,
  onValidationChange,
}) => {
  ...
  const postSaleValid = postSaleValidation(
    t,
    postsaleDetails,
    presaleTokenDetails?.pricePerToken
  );
  const infoValid = informationValidation(t, information);
  const datesValid = dateSelectionValidation(
    t,
    datesSelected,
    settings?.maxIazoLength,
    settings?.minIazoLength
  );
