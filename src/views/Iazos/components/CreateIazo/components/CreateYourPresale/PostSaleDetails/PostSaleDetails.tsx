...
const PostSaleDetails: React.FC<PostSaleDetailsProp> = ({
  quoteTokenSymbol,
  presalePrice,
  onChange,
}) => {
  ...
  const [liquidityDetails, setLiquidityDetails] =
    useState<LiquidityLockDetails>({
  ...
  const onLiquidityClick = (amount: number) => {
    setLiquidityDetails((prevState) => ({
      ...prevState,
      liquidityPercent: amount,
    }));
  };
  ...
  return (
    <>
      <LaunchPadInfoWrapper>
        ...
        <PercentageToRaiseWrapper>
          <InputTitle>
            {t("Percentage of raise to lock in liquidity")}
          </InputTitle>
          <LiquidityButton
            active={liquidityDetails?.liquidityPercent === 0.3}
            onClick={() => onLiquidityClick(0.3)}
          >
            30%
          </LiquidityButton>
          <LiquidityButton
            active={liquidityDetails?.liquidityPercent === 0.5}
            onClick={() => onLiquidityClick(0.5)}
          >
            50%
          </LiquidityButton>
          <LiquidityButton
            active={liquidityDetails?.liquidityPercent === 0.75}
            onClick={() => onLiquidityClick(0.75)}
          >
            75%
          </LiquidityButton>
          <LiquidityButton
            active={liquidityDetails?.liquidityPercent === 1}
            onClick={() => onLiquidityClick(1)}
          >
            100%
          </LiquidityButton>
        </PercentageToRaiseWrapper>
        <InputsWrapper>
          <TokenInput
            onChange={(e) =>
              setLiquidityDetails({
                ...liquidityDetails,
                listingPrice: e.currentTarget.value,
              })
            }
          ...
          <DropdownList
            onChange={(item) =>
              setLiquidityDetails({
                ...liquidityDetails,
                lockLiquidity: lockedLiquidityValues[item],
              })
            }
