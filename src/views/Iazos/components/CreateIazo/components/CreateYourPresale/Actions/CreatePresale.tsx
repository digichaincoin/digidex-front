...
import { AutoRenewIcon } from "@digidexfinance/uikit";
...
const CreatePresale: React.FC<CreatePresaleProps> = ({
  presaleData,
  disabled,
  creationFee,
  iazoFee,
  ...
  const { burnRemains, pricePerToken, softcap, limitPerUser, tokensForSale } =
    presaleTokenDetails;
  const {
    website,
    whitepaper,
  ...
  const quoteTokenObject: Token = tokens.find(
    (token) => token.symbol.toLowerCase() === quoteToken.toLowerCase()
  );
  ...
  const formattedPricePerToken = formatTokenPriceToBaseToken
    .times(new BigNumber(10).pow(18 - tokenDecimals))
    .toString();
  ...
  const formattedHardcap = new BigNumber(tokensForSale)
  ...
  const formattedLiquidityPercent =
    liquidityPercent === 1
      ? liquidityPercent * 1000 - parseFloat(iazoFee)
  ...
  const { onCreateIazo } = useCreateIazo(
    tokenAddress,
    quoteTokenAddress,
    burnRemains,
    unitParams,
    ...
    <>
      <StyledButton
        onClick={async () => {
          ...
          await onCreateIazo()
            .then((resp) => {
              const iazoAddress = resp.events.find(
                (event) => event.event === "IAZOCreated"
              ).args.newIAZO;
              ...
              onCreateIazoApi(apiObject).then((apiResp: any) => {
                if (apiResp.status === 201) {
                  ...
                  toastError(
                    t(
                      "SS-IAO Error: Please contact the ApeSwap team for assistance."
                    )
                  );
                }
              });
