...
const SaleReview: React.FC<SaleReviewProps> = ({
  ...
  const formatedTotalSupply = getBalanceNumber(
  ...
  const tokensForOther =
    formatedTotalSupply -
    parseFloat(tokensForSale) -
    tokensForLiquidity -
    tokensForFees;

  // Amount raised chart details
  const amountRaisedInQuoteToken =
    parseFloat(pricePerToken) * parseFloat(tokensForSale);
  ...
  const amountForOther =
    liquidityPercent === 1
      ? 0
      : amountRaisedInQuoteToken - quoteTokenForLiqudity - quoteTokenForFees;
