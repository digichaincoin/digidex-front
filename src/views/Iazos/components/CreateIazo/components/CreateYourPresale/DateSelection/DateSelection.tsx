...
const formatCountdown = (t, startDate, endDate, duration?) => {
  ...
  return `${duration ? t("Last for") : t("Starts in")} ${timeUntil?.months} ${t(
    "months"
  )} ${timeUntil?.days} ${t("days")} ${timeUntil?.hours} ${t("hours")}`;
};

const DateSelection: React.FC<DateSelectorProps> = ({ onChange }) => {
  const delayedStartDate = new Date(
    new Date().setDate(new Date().getDate() + 3.001)
  );
  const delayedEndDate = new Date(
    new Date().setDate(new Date().getDate() + 5.001)
  );
  const [dateState, setDateState] = useState<DateObject>({
    start: delayedStartDate,
    end: delayedEndDate,
  });
    ...
    <DateContainer>
      ...
      <DateSelectionContainer>
        <TextContainer>
          ...
          <StyledSubText>
            {formatCountdown(t, new Date(), dateState.start)}
          </StyledSubText>
        </TextContainer>
        <DateButtonContainer>
          <DateSelectionButton
            onChange={(date) => setDateState({ ...dateState, start: date })}
          />
        </DateButtonContainer>
      </DateSelectionContainer>
