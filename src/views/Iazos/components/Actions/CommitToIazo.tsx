...
import { AutoRenewIcon, useMatchBreakpoints } from "@digidexfinance/uikit";
...
const CommitToIazo: React.FC<ApproveCreateIazoProps> = ({
  ...
  const userBalanceFormatted = getBalanceNumber(
    userBalance,
    parseInt(decimals)
  );
  ...
  const { onCommit } = useCommitToIazo(
    iazoAddress,
    new BigNumber(amountToCommit)
      .times(new BigNumber(10).pow(parseInt(decimals)))
      .toString(),
    isNative
  );
  return (
    <Wrapper>
      <TokenInput
        ...
        max={
          maxSpendFormatted < userBalanceFormatted
            ? maxSpendFormatted
            : userBalanceFormatted
        }
        min={0}
      />
      <BoldAfterText boldContent={`${maxSpendFormatted.toString()} ${symbol}`}>
        Max commitment:{" "}
      </BoldAfterText>
      <br />
      <StyledButton
