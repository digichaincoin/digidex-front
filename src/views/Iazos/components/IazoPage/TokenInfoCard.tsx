...
import { Text, useMatchBreakpoints, Skeleton } from "@digidexfinance/uikit";
...
const TokenInfoCard: React.FC<TokenInfoCardProps> = ({
  ...
  const formatTokenAddress = `${tokenAddress?.slice(
    0,
    5
  )}...${tokenAddress?.slice(tokenAddress?.length - 3, tokenAddress?.length)}`;
    ...
    <IazoCardWrapper>
      ...
      <TokenHeaderInformationWrapper>
        <TokenName color="white">
          {" "}
          {tokenName || <Skeleton width="200px" height="35px" />}{" "}
        </TokenName>
        <TokenButtonsWrapper>
