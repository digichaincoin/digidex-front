...
const InfoTab: React.FC<InfoTabProps> = ({ iazo }) => {
  ...
  const tokenTotalSupply = getBalanceNumber(
  ...
  const priceDifference = Math.abs(
    parseFloat(tokenPrice) / parseFloat(listingPrice)
  );
  const { t } = useTranslation();

  // Inputs
  const tokensForSale = getBalanceNumber(
    new BigNumber(amount),
    parseInt(decimals)
  );
  const tokensForLiquidity =
    tokensForSale * priceDifference * liquidityPercentParsed;
  const tokensForFees = tokenFee * tokensForSale;
  const tokensForOther =
    tokenTotalSupply - tokensForSale - tokensForLiquidity - tokensForFees;
  const items = [
    {
      label: t("For sale"),
      value: tokensForSale,
  ...
  return (
    <InfoWrapper>
      ...
      <div>
        <BoldAfterText boldContent={tokenTotalSupply.toString()}>
          {" "}
          {t("Total Token Supply")}:{" "}
        </BoldAfterText>
        <BoldAfterText boldContent={tokensForSale.toString()}>
          {" "}
          {t("Total Tokens For Sale")}:{" "}
        </BoldAfterText>
        <BoldAfterText boldContent={tokensForLiquidity.toFixed(2)}>
          {" "}
          {t("Total Tokens For Liquidity")}:{" "}
        </BoldAfterText>
      </div>
      <br />
      <br />
