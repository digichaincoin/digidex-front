...
import { Skeleton } from "@digidexfinance/uikit";
...
import useFetchUserIazoCommit, {
  UserCommit,
} from "views/Iazos/hooks/useFetchUserIazoCommit";
...
const DuringSale: React.FC<BeforeSaleProps> = ({
  ...
  const { deposited, tokensBought }: UserCommit = useFetchUserIazoCommit(
    iazoAddress,
    pendingUserInfo
  );
  const tokensDepositedFormatted = getBalanceNumber(
    new BigNumber(deposited),
    parseInt(decimals)
  );
  const tokensBoughtFormatted = getBalanceNumber(
    new BigNumber(tokensBought),
    parseInt(iazoToken.decimals)
  );
  const baseCollectedFormatted = getBalanceNumber(
    new BigNumber(totalBaseCollected),
    parseInt(decimals)
  );
  const hardcapFormatted = getBalanceNumber(
    new BigNumber(hardcap),
    parseInt(decimals)
  );
  const maxSpendFormatted = getBalanceNumber(
    new BigNumber(maxSpend),
    parseInt(decimals)
  );
    ...
    <Wrapper>
      ...
      <ProgressBarWrapper>
        <ProgressBar>
          <Progress
            percentComplete={`${
              percentRaised.toString() === "NaN" ? 0 : percentRaised
            }%`}
          />
        </ProgressBar>
      </ProgressBarWrapper>
      ...
      {tokensDepositedFormatted > 0 && (
        <>
          <br />
          <BoldAfterTextLarge
            boldContent={`${tokensBoughtFormatted.toString()} ${
              iazoToken.symbol
            }`}
          >
            {t("Tokens bought")}:{" "}
          ...
          <BoldAfterTextLarge
            boldContent={`${tokensDepositedFormatted.toString()} ${symbol}`}
          >
      ...
      {account ? (
        <Actions
          ...
          disabled={
            percentRaised >= 100 ||
            tokensDepositedFormatted === maxSpendFormatted
          }
      ...
      <IazoSymbolsContainer>
        <IazoSymbols
          iconImage="dollar"
          title={`${tokenPrice} ${symbol}`}
          description={t("Presale price")}
        />
        <IazoSymbols
          iconImage="lock"
