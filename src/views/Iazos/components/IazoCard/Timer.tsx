...
const formatCountdown = (countdown: any): string => {
  const formatDays =
    countdown.days < 10 ? `0${countdown.days}` : countdown.days.toString();
  const formatHours =
    countdown.hours < 10 ? `0${countdown.hours}` : countdown.hours.toString();
  const formatMinutes =
  ...
  const formatSeconds =
    countdown.seconds < 9
      ? `0${countdown.seconds.toFixed(0)}`
      : countdown.seconds.toFixed(0);
...
const Timer: React.FC<TimerProps> = ({ timeInfo, fontSize, fontColor }) => {
  ...
  const timeUntilStartFormatted = formatCountdown(
    getTimePeriods(timeUntilStart, true)
  );
  const timeUntilEndFormatted = formatCountdown(
    getTimePeriods(timeUntilEnd, true)
  );
  ...
  const timeToDisplay = () => {
    if (timeUntilStart > 0) {
      return (
        <BoldAfterText
          fontSize={fontSize}
          boldContent={timeUntilStartFormatted}
          fontColor={fontColor}
        >
          {t("Starts in")}{" "}
        </BoldAfterText>
    ...
    if (timeUntilEnd > 0) {
      return (
        <BoldAfterText
          fontSize={fontSize}
          boldContent={timeUntilEndFormatted}
          fontColor={fontColor}
        >
          {t("Ends in")}{" "}
        </BoldAfterText>
...
const BoldAfterText = styled(Text)<{
  boldContent?: string;
  fontSize?: string;
  fontColor?: string;
}>`
