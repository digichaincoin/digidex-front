...
import { useMatchBreakpoints } from "@digidexfinance/uikit";
...
const IazoCard: React.FC<iazoCardProps> = ({ iazo }) => {
  ...
  const maxSpend = getBalanceNumber(
    new BigNumber(maxSpendPerBuyer),
    parseInt(baseToken.decimals)
  ).toString();
  const totalRaiseFormated = getBalanceNumber(
    new BigNumber(status.totalBaseCollected),
    parseInt(baseToken.decimals)
  );
  const softcapFormated = getBalanceNumber(
    new BigNumber(softcap),
    parseInt(baseToken.decimals)
  );
    ...
    <IazoCardWrapper>
      ...
      <TopBodyWrapper>
        <TextBoxWrapper align="flex-start">
          <BoldAfterText
            boldContent={`${baseToken.symbol} / ${iazoToken.symbol}`}
          />
          <BoldAfterText boldContent={`${liqudiityLock}%`}>
            {t("Liquidity Lock")}:{" "}
          </BoldAfterText>
        </TextBoxWrapper>
        {!isMobile && (
          <TextBoxWrapper justify="flex-end" padding="15px">
            <BoldAfterText boldContent={`${maxSpend} ${baseToken.symbol}`}>
              {t("Max Spend")}{" "}
            </BoldAfterText>
          </TextBoxWrapper>
        )}
        <TextBoxWrapper align="flex-end">
          ...
          <BoldAfterText boldContent={`${softcapFormated} ${baseToken.symbol}`}>
            {t("Soft Cap")}:{" "}
          </BoldAfterText>
        </TextBoxWrapper>
      </TopBodyWrapper>
      <BottomBodyWrapper>
        ...
        <BoldAfterText
          boldContent={`${totalRaiseFormated} / ${hardcapFormatted} ${baseToken.symbol}`}
        />
      </BottomBodyWrapper>
    </IazoCardWrapper>
  );
