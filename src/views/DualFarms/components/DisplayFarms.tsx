...
import { Flex, Text, LinkExternal, Svg, useModal } from "@digidexfinance/uikit";
import { TagVariants } from "@digi.swap/uikit";
...
import { Svg as Icon } from "@digi.swap/uikit";
import { selectOutputCurrency } from "state/zap/actions";

const DisplayFarms: React.FC<{
  farms: DualFarm[];
  openPid?: number;
  dualFarmTags: Tag[];
}> = ({ farms, openPid, dualFarmTags }) => {
  ...
  const farmsListView = farms.map((farm, i) => {
    ...
    const userEarningsMiniChef = getBalanceNumber(
      farm?.userData?.miniChefEarnings || new BigNumber(0)
    ).toFixed(2);
    const userEarningsRewarder = getBalanceNumber(
      farm?.userData?.rewarderEarnings || new BigNumber(0)
    ).toFixed(2);
    const userEarningsUsd = `$${(
      getBalanceNumber(farm?.userData?.miniChefEarnings || new BigNumber(0)) *
        farm?.rewardToken0Price +
      getBalanceNumber(farm?.userData?.rewarderEarnings || new BigNumber(0)) *
        farm?.rewardToken1Price
    ).toFixed(2)}`;
    const userTokenBalance = `${getBalanceNumber(
      farm?.userData?.tokenBalance || new BigNumber(0)
    )?.toFixed(6)} LP`;

    const lpValueUsd = farm?.stakeTokenPrice;

    const userTokenBalanceUsd = `$${(
      getBalanceNumber(farm?.userData?.tokenBalance || new BigNumber(0)) *
      lpValueUsd
    ).toFixed(2)}`;
    ...
    return {
      ...
      tokens: {
        ...
        token4:
          farm?.dualImage !== false
            ? farm.pid === 11
              ? "NFTY2"
              : farm?.rewardTokens?.token1?.symbol
            : null,
      },
      stakeLp: true,
      title: (
        <Text ml={fTag?.pid === farm?.pid ? 0 : 10} bold>
          {farm?.stakeTokens?.token1?.symbol}-
        ...
        <>
          <Flex flexDirection="column">
            ...
            >
              ...
              <Text bold style={{ fontSize: "12px" }}>
                {farm?.stakeTokens?.token1?.symbol}-
                {farm?.stakeTokens?.token0?.symbol} LP
              </Text>
            </Flex>
      ...
      cardContent: (
        <>
          <ListViewContent
            title={t("APY")}
            value={
              parseFloat(farm?.apy) > 1000000 ? `>1,000,000%` : `${farm?.apy}%`
            }
            width={isMobile ? 90 : 150}
            ml={20}
            toolTip={t(
              "APY includes annualized DIGICHAIN rewards and rewards for providing liquidity (DEX swap fees), compounded daily."
            )}
          ...
          <ListViewContent
            ...
            valueIcon={
              <span style={{ marginRight: "5px" }}>
                <Svg icon="digichain_token" width={15} color="text" />
              </span>
            }
            width={isMobile ? 100 : 180}
            toolTip={t(
              "DIGICHAIN reward APRs are calculated in real time. DEX swap fee APRs are calculated based on previous 24 hours of trading volume. Note: APRs are provided for your convenience. APRs are constantly changing and do not represent guaranteed returns."
            )}
            toolTipPlacement={
              i === farms.length - 1 && i !== 0 ? "topLeft" : "bottomLeft"
            }
            toolTipTransform={
          ...
          <ListViewContent
            ...
            width={isMobile ? 100 : 180}
            toolTip={t(
              "The total value of the LP tokens currently staked in this farm."
            )}
            toolTipPlacement={isMobile ? "bottomRight" : "bottomLeft"}
            toolTipTransform={
              isMobile ? "translate(13%, 0%)" : "translate(23%, 0%)"
            }
          />
          <ListViewContent
            title={t("Earned")}
            ...
            valueIcon={
              <ServiceTokenDisplayContainer>
                <ServiceTokenDisplay
                  token1={farm?.rewardTokens.token0.symbol}
                  size={15}
                />
              </ServiceTokenDisplayContainer>
            }
      ...
      expandedContent: (
        <>
          ...
          <HarvestAction
            pid={farm.pid}
            disabled={userEarningsMiniChef === "0.00"}
