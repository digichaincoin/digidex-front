...
import { Text, Image, LinkExternal } from "@digidexfinance/uikit";
...
export const ToggleButton = styled.button`
  ...
  border: none;
  color: ${({ theme }) =>
    theme.isDark ? theme.colors.primaryBright : theme.colors.primary};
  cursor: pointer;
  display: block;
  font-weight: 600;
  outline: 0;
...
export const Description = styled(Text).attrs({ fontFamily: "poppins" })<{
  isOpen: boolean;
}>`
  color: ${({ theme }) =>
    theme.isDark ? theme.colors.primaryBright : theme.colors.primary};
