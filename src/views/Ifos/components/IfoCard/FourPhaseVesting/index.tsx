...
const IfoCard: React.FC<IfoCardProps> = ({ ifo, gdigi }) => {
  ...
  const { userTokenStatus, userInfo, offeringTokenBalance } = useUserInfo(
    contract,
    tokenDecimals,
    address
  );
  ...
  const stats = React.useMemo(() => {
    ...
    if (
      ...
      const totalTokensHarvested =
        getBalanceNumber(offeringTokenBalance, tokenDecimals) -
        (tokensVested + tokensHarvestedAvailable);
      const vestedValueAmount = userInfo.refunded
        ? state.raisingAmount.times(userInfo.allocation)
        : userInfo.amount;
      const vestedValueDollar = getBalanceNumber(
        vestedValueAmount.times(currencyPrice),
        18
      ).toFixed(2);
      ...
      texts = [
        ...
        { label: t("Tokens vesting"), value: tokensVested.toFixed(4) },
        {
          ...
          value: totalTokensHarvested.toFixed(4),
        },
        {
          label: t("Committed value"),
          value: `${Number(getBalanceNumber(vestedValueAmount, 18)).toFixed(
            4
          )} ${currency} (~$${vestedValueDollar})`,
        },
      ];
