...
const IfoCardContribute: React.FC<Props> = ({
  ...
  if (
    ...
    (allowance.isLessThanOrEqualTo(new BigNumber("0")) ||
      allowance.isLessThan(tokenBalance))
  ) {
    return (
      <ApproveButton
        disabled={pendingTx}
  ...
  const amountContributed = getBalanceNumber(
    new BigNumber(userInfo.amount.toString())
  );
        ...
        <>
          <VestingButtonWrapper>
            {amountContributed > 0 && (
              <>
                <VestingClaimButton
                  disabled={userHarvestedFlags[0]}
                  onClick={() => onClaim(0)}
                >
                  {userHarvestedFlags[0] ? (
                    <Claim>Claimed</Claim>
                  ) : (
                ...
                {(tokensVested > 0 || tokensHarvestedAvailable > 0) && (
                  <>
                    <VestingClaimButton
                      disabled={
                        harvestBlockReleases.two > 0 || userHarvestedFlags[1]
                      }
                      onClick={() => onClaim(1)}
                    >
                      {userHarvestedFlags[1] &&
                        harvestBlockReleases.two < 0 && (
                          <Claim>{t("Claimed")}</Claim>
                        )}
                      {!userHarvestedFlags[1] &&
                        harvestBlockReleases.two < 0 && (
                          <Claim color="white">{t("Claim")}</Claim>
                      ...
                      {harvestBlockReleases.two > 0 && (
                        <>
                          <DisplayVestingTime label>
                            {t("Vesting time")}
                          </DisplayVestingTime>
                          <DisplayVestingTime>
                            {formatTime(harvestTwoTime)}
                          </DisplayVestingTime>
                        </>
                      )}
                    </VestingClaimButton>
                    <VestingClaimButton
                      disabled={
                        harvestBlockReleases.three > 0 || userHarvestedFlags[2]
                      }
                      onClick={() => onClaim(2)}
                    >
                      {userHarvestedFlags[2] &&
                    ...
                    <VestingClaimButton
                      disabled={
                        harvestBlockReleases.four > 0 || userHarvestedFlags[3]
                      }
                      onClick={() => onClaim(3)}
                    >
                      {userHarvestedFlags[3] &&
