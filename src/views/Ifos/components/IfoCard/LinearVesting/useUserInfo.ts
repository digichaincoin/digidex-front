...
function useUserInfo(
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      try {
        const [userTokens, userInfos, refundingAmount, offeringAmount] =
          await multicall(chainId, ifoLinearAbi, calls);
        setUserTokenStatus({
          stakeTokenHarvest: getBalanceNumber(
            new BigNumber(userTokens?.stakeTokenHarvest.toString()),
            tokenDecimals
          ),
        ...
        setUserInfo({
          ...
          refundingAmount: getBalanceNumber(
            new BigNumber(refundingAmount.toString())
          ),
          offeringAmount: getBalanceNumber(
            new BigNumber(offeringAmount.toString()),
            tokenDecimals
          ),
          ...
          lastBlockHarvested: new BigNumber(
            userInfos?.offeringTokenInitialHarvest?.toString()
          ).toNumber(),
