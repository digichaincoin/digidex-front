...
const ContributeInputComponent: React.FC<Props> = ({
  ...
  const { pendingTx, handleDeposit, isAmountValid } = useIAODeposit(
    contract,
    currencyAddress,
    tokenBalance
  );
  ...
  const useMax = () => {
    ...
    setValue(
      currency === "BNB"
        ? (bigBalance > bnbReduction
            ? bigBalance.minus(bnbReduction).toFixed()
            : 0
          ).toString()
        : balance
    );
  ...
  return (
    <Flex sx={{ alignItems: "flex-end" }}>
      ...
      />
      <Flex
        sx={{ flexWrap: "wrap", justifyContent: "flex-end", minWidth: "130px" }}
      >
