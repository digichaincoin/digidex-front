import { IconButton, Text, Flex, TagVariants, Svg } from "@digi.swap/uikit";
...
import { useModal } from "@digidexfinance/uikit";
...
const DisplayJungleFarms: React.FC<{
  jungleFarms: JungleFarm[];
  openId?: number;
  jungleFarmTags: Tag[];
}> = ({ jungleFarms, openId, jungleFarmTags }) => {
  ...
  const jungleFarmsListView = jungleFarms.map((farm) => {
    ...
    const userEarningsUsd = `$${(
      userEarnings * farm.rewardToken?.price
    ).toFixed(2)}`;
    const userTokenBalance = `${getBalanceNumber(
      farm?.userData?.stakingTokenBalance || new BigNumber(0)
    )?.toFixed(6)}`;
    const userTokenBalanceUsd = `$${(
      getBalanceNumber(
        farm?.userData?.stakingTokenBalance || new BigNumber(0)
      ) * farm?.stakingToken?.price
    ).toFixed(2)}`;
            ...
            <StyledButton
              onClick={() =>
                showLiquidity(
                  ...
                  farm?.lpTokens?.quoteToken?.symbol === "BNB"
                    ? "ETH"
