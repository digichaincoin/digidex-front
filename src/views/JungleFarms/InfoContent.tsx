...
const InfoContent: React.FC<{ farm: JungleFarm }> = ({ farm }) => {
  ...
  const timeUntilStart = getTimePeriods(
    Math.max(farm?.startBlock - currentBlock, 0) * BSC_BLOCK_TIME,
    true
  );
  const timeUntilEnd = getTimePeriods(
    Math.max(farm?.endBlock - currentBlock, 0) * BSC_BLOCK_TIME,
    true
  );
  ...
  return (
    <>
      <Flex flexDirection="column">
        {farm?.endBlock > 0 && farm?.rewardToken?.symbol !== "DIGICHAIN" && (
          <Flex
          ...
          >
            <Text style={{ fontSize: "14px" }}>
              {farm?.startBlock > currentBlock ? "Starts in" : "Ends in"}
            </Text>
