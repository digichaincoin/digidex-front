...
const JungleFarms: React.FC = () => {
  ...
  const currJungleFarms = allJungleFarms.map((farm) => {
    return {
      ...farm,
      isFinished: farm.isFinished || currentBlock > farm.endBlock,
    };
  });
  ...
  const [finishedJungleFarms, openFarms] = partition(
    currJungleFarms,
    (farm) => farm.isFinished
  );
    ...
    switch (sortOption) {
      ...
      case "liquidity":
        return orderBy(
          farmsToSort,
          (farm: JungleFarm) =>
            getBalanceNumber(farm.totalStaked) * farm.stakingToken.price,
          "desc"
        );
      case "earned":
        return orderBy(
          ...
          (farm: JungleFarm) => {
            ...
            return (
              getBalanceNumber(farm.userData.pendingReward) *
              farm.rewardToken?.price
            );
          },
          "desc"
        );
        ...
        return jungleFarmTags
          ? orderBy(
              ...
              (farm: JungleFarm) =>
                jungleFarmTags?.find(
                  (tag) =>
                    tag.pid === farm.jungleId &&
                    tag.text.toLowerCase() === "hot"
                ),
              "asc"
            )
          : farmsToSort;
      ...
      default:
        return jungleFarmOrderings
          ? orderBy(
              farmsToSort,
              (farm: JungleFarm) =>
                jungleFarmOrderings?.find(
                  (ordering) => ordering.pid === farm.jungleId
                )?.order,
              "asc"
            )
          : farmsToSort;
  ...
  const renderJungleFarms = () => {
    ...
    if (searchQuery) {
      ...
      chosenJungleFarms = chosenJungleFarms.filter((farm) =>
        farm.tokenName.toLowerCase().includes(lowercaseQuery)
      );
    }
      ...
      >
        <ListViewLayout>
          <Banner
            ...
            link="https://digichain.gitbook.io/digidex-finance/product-and-features/stake/farms"
            listViewBreak
            maxWidth={1130}
          />
          <Flex alignItems="center" justifyContent="center" mt="20px">
            <ListViewMenu
              ...
              harvestAll={
                <HarvestAll
                  jungleIds={harvestJungleIds}
                  disabled={harvestJungleIds.length === 0}
                />
              }
