...
import { Text, Flex, Link, LinkExternal } from "@digidexfinance/uikit";
...
const Detail: React.FC<ExpandableSectionProps> = ({
  ...
  const totalStakedTitle =
    type === "card" ? t("Total Staked Value") : t("Total Staked");
        ...
        <>
          <Flex justifyContent="space-between">
            ...
            <StyledText fontSize="12px">
              {rewardToken.symbol === "DIGICHAIN"
                ? t("Never")
                : `${timeUntilEnd.days + timeUntilEnd.months * 30}d, ${
                    timeUntilEnd.hours
                  }h, ${timeUntilEnd.minutes}m`}
            </StyledText>
          </Flex>
        </>
      )}
        ...
        <StyledLink
          ...
          onClick={() =>
            registerToken(
              ...
              `${URLactual.origin}/images/tokens/${
                imageToken || `${rewardToken?.symbol}.svg`
              }`
            )
          }
        >
