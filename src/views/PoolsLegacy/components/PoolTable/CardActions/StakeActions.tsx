...
const StakeAction: React.FC<StakeActionsProps> = ({
  ...
  const onUnstake = useReward(
    rewardRefUnstake,
    useSousUnstake(sousId).onUnstake
  );
  ...
  if (firstStake) {
    return (
      <StyledButton onClick={onPresentDeposit}>
        {t("STAKE %stakingTokenName%", { stakingTokenName })}
      </StyledButton>
    );
  }
  ...
  return (
    <StyledFlex justifyContent="space-between">
      ...
      >
        ...
        >
          {rawStakedBalance > 0 && displayBalance === "0" ? (
            <>&gt;{displayBalance}</>
          ) : (
            displayBalance
          )}
      ...
      {isApproved && (
        <IconButtonWrapper>{renderStakingButtons()}</IconButtonWrapper>
      )}
    </StyledFlex>
  );
};
