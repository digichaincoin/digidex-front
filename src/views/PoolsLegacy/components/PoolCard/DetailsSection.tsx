...
const DetailsSection: React.FC<ExpandableSectionProps> = ({
  ...
  const totalDollarAmountStakedFormated = totalDollarAmountStaked
    ? `${Number(totalDollarAmountStaked).toLocaleString(undefined, {
  ...
  const totalUserStaked =
    personalValueStaked > 0
      ? (personalValueStaked * stakedTokenPrice).toFixed(2)
      : 0;
        ...
        <>
          <Flex justifyContent="space-between">
            ...
            <StyledText fontSize="12px">{`${
              timeUntilEnd.days + timeUntilEnd.months * 30
            }d, ${timeUntilEnd.hours}h, ${timeUntilEnd.minutes}m`}</StyledText>
          </Flex>
        </>
      )}
      <Flex justifyContent="space-between">
