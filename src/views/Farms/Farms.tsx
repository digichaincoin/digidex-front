...
const Farms: React.FC = () => {
  ...
  const hasHarvestPids = [...activeFarms, ...inactiveFarms]
    .filter(
      (farm) =>
        farm.userData && new BigNumber(farm.userData.earnings).isGreaterThan(0)
    )
    .map((filteredFarm) => {
    ...
    switch (sortOption) {
      case "all":
        return farmOrderings
          ? orderBy(
              farms,
              (farm: Farm) =>
                farmOrderings.find((ordering) => ordering.pid === farm.pid)
                  ?.order,
              "asc"
      ...
      case "stables":
        return farms
          .filter(
            (farm) =>
              STABLES.includes(farm.tokenSymbol) &&
              STABLES.includes(farm.quoteTokenSymbol)
          )
          .slice(0, numberOfFarmsVisible);
      case "apr":
        return orderBy(farms, (farm) => parseFloat(farm.apy), "desc").slice(
          0,
      ...
      case "blueChips":
        return farms
          .filter(
            (farm) =>
              BLUE_CHIPS.includes(farm.tokenSymbol) ||
              BLUE_CHIPS.includes(farm.quoteTokenSymbol)
          )
          .slice(0, numberOfFarmsVisible);
      case "liquidity":
        return orderBy(
          farms,
          (farm: Farm) => parseFloat(farm.totalLpStakedUsd),
          "desc"
        ).slice(0, numberOfFarmsVisible);
      case "hot":
        return farmTags
          ? orderBy(
              ...
              (farm: Farm) =>
                farmTags?.find(
                  (tag) =>
                    tag.pid === farm.pid && tag.text.toLowerCase() === "hot"
                ),
              "asc"
            ).slice(0, numberOfFarmsVisible)
          : farms.slice(0, numberOfFarmsVisible);
        ...
        return farmTags
          ? orderBy(
              ...
              (farm: Farm) =>
                farmTags?.find(
                  (tag) =>
                    tag.pid === farm.pid && tag.text.toLowerCase() === "new"
                ),
              "asc"
            ).slice(0, numberOfFarmsVisible)
          : farms.slice(0, numberOfFarmsVisible);
  ...
  useSetZapOutputList(
    activeFarms?.map((farm) => {
      return {
        currencyIdA: farm?.tokenAddresses[chainId],
        currencyIdB: farm?.quoteTokenAdresses[chainId],
      };
    })
  );
  ...
  return (
    <>
      ...
      >
        <ListViewLayout>
          <Banner
            banner="digichain-farms"
            link="?modal=1"
            title={t("Digichain Farm")}
            listViewBreak
            maxWidth={1130}
          />
