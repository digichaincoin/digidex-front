...
import { Text, Svg } from "@digidexfinance/uikit";
import { TagVariants, useModal } from "@digi.swap/uikit";
...
const DisplayFarms: React.FC<{
  ...
  const farmsListView = farms.map((farm) => {
    ...
    const userEarningsUsd = `$${(
      getBalanceNumber(farm?.userData?.earnings || new BigNumber(0)) *
      farm.digichainPrice
    ).toFixed(2)}`;
    ...
    return {
      ...
      cardContent: (
        <>
          ...
          <ListViewContent
            ...
            aprCalculator={
              <CalcButton
                ...
                rewardTokenPrice={farm.digichainPrice}
                ...
                quoteTokenAddress={
                  farm.quoteTokenSymbol === "BNB"
                    ? "ETH"
                    : farm.quoteTokenAdresses[chainId]
                }
                farm={farm}
              />
            }
          ...
          <ListViewContent
            title={t("Liquidity")}
            value={`$${Number(farm?.totalLpStakedUsd).toLocaleString(
              undefined
            )}`}
            width={isMobile ? 100 : 180}
            toolTip={t(
