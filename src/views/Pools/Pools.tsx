        ...
        return poolTags
          ? orderBy(
              ...
              (pool: Pool) =>
                poolTags?.find(
                  (tag) =>
                    tag.pid === pool.sousId && tag.text.toLowerCase() === "hot"
                ),
              "asc"
            )
          : poolsToSort;
        ...
        return poolTags
          ? orderBy(
              ...
              (pool: Pool) =>
                poolTags?.find(
                  (tag) =>
                    tag.pid === pool.sousId && tag.text.toLowerCase() === "new"
                ),
              "asc"
            )
          : poolsToSort;
      default:
        return poolOrderings
          ? orderBy(
              poolsToSort,
              (pool: Pool) =>
                poolOrderings?.find((ordering) => ordering.pid === pool.sousId)
                  ?.order,
              "asc"
            )
          : poolsToSort;
  ...
  const renderPools = () => {
    ...
    if (tokenOption !== "allTokens") {
      chosenPools = chosenPools.filter(
        (pool) => pool.stakingToken.symbol === tokenOption.toUpperCase()
      );
    }
      ...
      >
        <ListViewLayout>
          ...
          <Flex
            flexDirection="column"
            alignSelf="center"
            style={{ maxWidth: "1130px", width: "100%" }}
          >
            <PoolMenu
