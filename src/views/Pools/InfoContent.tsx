...
import { Flex, Text, LinkExternal } from "@digidexfinance/uikit";
...
const InfoContent: React.FC<{ pool: Pool }> = ({ pool }) => {
  ...
  const timeUntilStart = getTimePeriods(
    Math.max(pool?.startBlock - currentBlock, 0) * BSC_BLOCK_TIME,
    true
  );
  const timeUntilEnd = getTimePeriods(
    Math.max(pool?.endBlock - currentBlock, 0) * BSC_BLOCK_TIME,
    true
  );
  ...
  return (
    <>
      <Flex flexDirection="column">
        {pool?.endBlock > 0 && pool?.rewardToken?.symbol !== "DIGICHAIN" && (
          <Flex
          ...
          >
            <Text style={{ fontSize: "14px" }}>
              {pool?.startBlock > currentBlock ? t("Starts in") : t("Ends in")}
            </Text>
