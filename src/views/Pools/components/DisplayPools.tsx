...
import { IconButton, Text, Flex, TagVariants, Button } from "@digi.swap/uikit";
...
const DisplayPools: React.FC<{
  pools: Pool[];
  openId?: number;
  poolTags: Tag[];
}> = ({ pools, openId, poolTags }) => {
  ...
  const poolsListView = pools.map((pool) => {
    ...
    const liquidityUrl = !pool?.lpStaking
      ? pool?.stakingToken?.symbol === "GDIGI"
        ? "https://digidex.finance/gdigi"
        : `https://digidex.finance/swap?outputCurrency=${pool?.stakingToken?.address[chainId]}`
    ...
    const userEarningsUsd = `$${(
      userEarnings * pool.rewardToken?.price
    ).toFixed(2)}`;
    const userTokenBalance = `${getBalanceNumber(
      pool?.userData?.stakingTokenBalance || new BigNumber(0)
    )?.toFixed(6)}`;
    ...
    const openLiquidityUrl = () =>
      pool?.stakingToken?.symbol === "GDIGI"
        ? history.push({ search: "?modal=gdigi" })
      ...
      cardContent: (
        <>
          ...
          <ListViewContent
            ...
            height={50}
            toolTip={t(
              "The total value of the tokens currently staked in this pool."
            )}
