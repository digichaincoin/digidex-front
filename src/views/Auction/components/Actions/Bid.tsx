...
const CalculateMinBid = (
  ...
  const percentBidRaise =
    getBalanceNumber(new BigNumber(minBidPercent), 3) * rawBidAmount +
    rawBidAmount;
  const amountBidRaise =
    getBalanceNumber(new BigNumber(minBidRaise)) + rawBidAmount;
...
const Bid: React.FC<BidProps> = ({
  ...
  const rawMinBidRaise = CalculateMinBid(
    minBidRaise,
    minBidPercentage,
    rawBidAmount
  );
  ...
  return (
    <>
      <BidWrapper>
        ...
        <UserBalanceWrapper>
          {t("Balance: %balance%", { balance: rawBnbBalance })}{" "}
        </UserBalanceWrapper>
      </BidWrapper>
      <SubmitBid
