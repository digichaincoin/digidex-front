...
import { Text, ArrowBackIcon, ArrowForwardIcon } from "@digidexfinance/uikit";
...
const HistoryWrapper = styled.div`
  ...
  box-shadow: 5px 4px 8px rgba(0, 0, 0, 0.1),
    inset 355px 4px 250px rgba(255, 255, 255, 0.1);
      ...
      historyData?.slice(prevSlice, curSlice).map((data, i) =>
        !(i % 2) ? (
          <Row background>
            ...
            <Column>
              {getBalanceNumber(new BigNumber(data.amount)).toFixed(2)} BNB
            </Column>
            <Column>
              {data.bidder.slice(0, 4)}...
              {data.bidder.slice(data.bidder.length - 4, data.bidder.length)}
            </Column>
