import { Skeleton, Text, useMatchBreakpoints } from "@digidexfinance/uikit";
...
const VestedTimer: React.FC<{
  ...
  const vestingTime = getTimePeriods(
    parseInt(lastBlockTimestamp) + parseInt(vesting) - currentTime,
    true
  );
  ...
  return transferModalFlag ? (
    ...
    <ListViewContent
      ...
      height={52.5}
      toolTip={t(
        "This is the time remaining until all tokens from the bill are available to claim."
      )}
      toolTipPlacement={isMobile ? "bottomRight" : "bottomLeft"}
      toolTipTransform={
        isMobile ? "translate(-75%, 65%)" : "translate(0%, 65%)"
      }
    />
  );
};
