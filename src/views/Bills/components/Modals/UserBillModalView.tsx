...
import { Flex, Skeleton, Text } from "@digidexfinance/uikit";
import { Modal, useModal } from "@digi.swap/uikit";
...
const BuyBillModalView: React.FC<BillModalProps> = ({
  onDismiss,
  bill,
  billId,
}) => {
  ...
  const userOwnedBillNftData = userOwnedBillsNftData?.find(
    (b) => parseInt(b.tokenId) === parseInt(billId)
  );
  const pending = getBalanceNumber(
    new BigNumber(userOwnedBill?.payout),
  ...
  const claimableUsd = (parseFloat(claimable) * bill?.earnTokenPrice)?.toFixed(
    2
  );
      ...
      <Container>
        <ModalBodyContainer>
          ...
          {userOwnedBillNftData?.image ? (
            <BillsImage
              image={`${userOwnedBillNftData?.image + "?img-width=720"}`}
            />
          ) : (
          ...
          <BillDescriptionContainer minHeight={360}>
            ...
            <UserActionButtonsContainer>
              ...
              />
              <StyledButton
                onClick={onPresentTransferBillModal}
                style={{ width: "218px" }}
              >
                {t("Transfer")}
              </StyledButton>
