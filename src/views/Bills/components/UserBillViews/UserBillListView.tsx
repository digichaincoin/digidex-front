...
import { Flex, useMatchBreakpoints } from "@digidexfinance/uikit";
...
const UserBillListView: React.FC<{ bills: Bills[]; showAll?: boolean }> = ({
  bills,
  showAll,
}) => {
  ...
  const billsListView = bills.flatMap((bill) => {
    ...
    return ownedBills.flatMap((ownedBill) => {
      if (
        !showAll &&
        parseFloat(ownedBill.pendingRewards) === 0 &&
        parseFloat(ownedBill.payout) === 0
      ) {
        return [];
      }
      const pending = getBalanceNumber(
        new BigNumber(ownedBill.payout),
        ...
        cardContent: (
          <>
            <ListViewContent
              ...
              height={52.5}
              toolTip={t(
                "This is the amount of tokens that have vested and available to claim."
              )}
            ...
            <ListViewContent
              ...
              height={52.5}
              toolTip={t(
                "This is the amount of unvested tokens that cannot be claimed yet."
              )}
              toolTipPlacement="bottomLeft"
              toolTipTransform="translate(0, 65%)"
            />
            <VestedTimer
              lastBlockTimestamp={ownedBill.lastBlockTimestamp}
              vesting={ownedBill.vesting}
            />
            {!isMobile && (
              <>
