import { Flex, Skeleton } from "@digidexfinance/uikit";
...
const BillCard: React.FC<{ bills: Bills[]; ml?: string }> = ({ bills, ml }) => {
  ...
  const ownedBillsAmount = bills?.flatMap((bill) =>
    bill?.userOwnedBillsData ? bill?.userOwnedBillsData : []
  ).length;
  const billsCardView = bills
    .flatMap((bill) => {
      ...
      return ownedBills?.flatMap((ownedBill, i) => {
        ...
        const ownedBillNftData = bill?.userOwnedBillsNftData
          ? bill?.userOwnedBillsNftData[i]
          : null;
        return (
          <SwiperSlide
            style={{ maxWidth: "270px", height: "307px" }}
            key={ownedBill.id}
          >
            <CardContainer ml={ml} key={ownedBill.id}>
              {ownedBillNftData?.image ? (
                <BillModal
                  ...
                  billId={ownedBill.id}
                  billCardImage={`${
                    ownedBillNftData?.image + "?img-width=720"
                  }`}
                />
              ) : (
