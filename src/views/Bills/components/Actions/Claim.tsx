...
import { useModal } from "@digi.swap/uikit";
...
const Claim: React.FC<ClaimProps> = ({
  ...
  const [onPresentGHModal] = useModal(
    <CircularModal actionType={MODAL_TYPE.GENERAL_HARVEST} />,
    true,
    true,
    "ghModal"
  );
  ...
  const digichainBill = billType === "bill";
  ...
  const handleClaim = async () => {
    ...
    await onClaimBill()
      .then((resp) => {
        ...
        if (digichainBill) displayGHCircular();
      })
      .catch((e) => {
        console.error(e);
        setPendingTrx(false);
