...
import { ZapType } from "@digi.swap/sdk";
import {
  useDerivedZapInfo,
...
const Buy: React.FC<BuyProps> = ({ bill, onBillId, onTransactionSubmited }) => {
  ...
  const bigValue = new BigNumber(consideredValue).times(
  ...
  // threshold equals to 10 usd in earned tokens (digichain or jungle token)
          ...
          track({
            ...
            data: {
              ...
              amount: getBalanceNumber(
                new BigNumber(zap.currencyIn.inputAmount.toString())
              ),
            },
          });
        })
        ...
        <BillValueContainer>
          <TextWrapper>
            <Text size="12px" pr={1}>
              ...
              <span style={{ fontWeight: 700 }}>
                {billValue === "NaN" ? "0" : parseFloat(billValue).toFixed(3)}{" "}
                {earnToken?.symbol}
              </span>
            </Text>
          </TextWrapper>
          <TextWrapper>
            <Text size="12px">
              {t("Available")}:{" "}
              <span style={{ fontWeight: 700 }}>
                {!available ? "0" : new BigNumber(safeAvailable).toFixed(3)}{" "}
                {earnToken?.symbol}
              </span>
            </Text>
          </TextWrapper>
