import { Button, Flex, Svg, Text } from "@digi.swap/uikit";
import { Currency, ZapType } from "@digi.swap/sdk";
...
const ZapTest: React.FC = () => {
  ...
  const { INPUT, OUTPUT, independentField, typedValue, recipient, zapType } =
    useZapState();

  const { zap, parsedAmount, inputError: zapInputError } = useDerivedZapInfo();

  const { onCurrencySelection, onUserInput, onSetZapType } =
    useZapActionHandlers();

  const inputCurrency = useCurrency(INPUT?.currencyId);
  const outputCurrencys = [
    useCurrency(OUTPUT?.currency1),
    useCurrency(OUTPUT?.currency2),
  ];
  ...
  const billExists = useMemo(
    () =>
      bills.find(
        (bill) =>
          bill.lpToken.address[56].toLowerCase() ===
          pairOut?.pair?.liquidityToken?.address.toLowerCase()
      ),
    [bills, pairOut]
  );
      ...
      >
        ...
        >
          <Button
            onClick={() =>
              onSetZapType(
                zapType === ZapType.ZAP ? ZapType.ZAP_T_BILL : ZapType.ZAP
              )
            }
          >
        ...
        <DexPanel
          ...
          onCurrencySelect={(field, currency) =>
            handleZapCurrencySelection(field, [currency])
          }
        ...
        <Flex sx={{ minWidth: "400px", maxWidth: "600px" }}>
          <Flex sx={{ minWidth: "200px" }}>
            <DexPanel
              ...
              onCurrencySelect={(field, currency) =>
                handleZapCurrencySelection(field, [
                  currency,
                  outputCurrencys[1],
                ])
              }
        ...
        {!showApproveFlow ? (
          <Button
            ...
            onClick={zapCallback}
            disabled={
              pairOut.pairState === PairState.NOT_EXISTS ||
              (zapType === ZapType.ZAP_T_BILL && !billExists)
            }
          >
        ...
        <Flex sx={{ mt: "10px", flexDirection: "column" }}>
          ...
          {zapType === ZapType.ZAP_T_BILL && (
            <Text>Does Bill Exist?: {billExists ? "Yes" : "No"}</Text>
          )}
          <Text>
            Estimated Zap LP amount:{" "}
            {pairOut.liquidityMinted?.toSignificant(10)}
          </Text>
