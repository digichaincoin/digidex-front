...
const Vaults: React.FC = () => {
  ...
  const [numberOfVaultsVisible, setNumberOfVaultsVisible] = useState(
    NUMBER_OF_VAULTS_VISIBLE
  );
  ...
  useEffect(() => {
    const showMorePools = (entries) => {
      ...
      if (entry.isIntersecting) {
        setNumberOfVaultsVisible(
          (vaultsCurrentlyVisible) =>
            vaultsCurrentlyVisible + NUMBER_OF_VAULTS_VISIBLE
        );
      }
    };
  ...
  const [inactiveVaults, activeVaults] = partition(
    allVaults,
    (vault) => vault.inactive
  );
    ...
    switch (sortOption) {
      ...
      case "totalStaked":
        return orderBy(
          vaultsToSort,
          (vault: Vault) =>
            parseInt(vault?.totalStaked) * vault?.stakeTokenPrice,
          "desc"
        );
      default:
  ...
  const renderVaults = (): Vault[] => {
    ...
    if (vaultType !== "allTypes") {
      chosenVaults = chosenVaults.filter(
        (vault) => vault.type === vaultType.toUpperCase()
      );
    }
  ...
  useSetZapOutputList(
    activeVaults?.slice(1).map((vault) => {
      return {
        currencyIdA: vault?.token?.address[chainId],
        currencyIdB: vault?.quoteToken?.address[chainId],
      };
    })
  );
  ...
  return (
    <>
      ...
      >
        <ListViewLayout>
          <Banner
            title={t("DIGICHAIN Maximizers BOOST")}
            banner="digichain-maximizers"
            link="https://digichain.gitbook.io/digidex-finance/product-and-features/stake/vaults"
