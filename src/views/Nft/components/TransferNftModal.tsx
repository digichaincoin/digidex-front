...
import { Button, Input, Modal, Text } from "@digidexfinance/uikit";
import { Nft } from "config/constants/types";
import { useTranslation } from "contexts/Localization";
import { useNonFungibleDigis } from "hooks/useContract";
...
const TransferNftModal: React.FC<TransferNftModalProps> = ({
  ...
  const nonFungibleDigisContract = useNonFungibleDigis();
  ...
  const handleConfirm = async () => {
    try {
      ...
      if (!isValidAddress) {
        ...
        setIsLoading(true);
        await nonFungibleDigisContract[
          "safeTransferFrom(address,address,uint256)"
        ](account, value, tokenId).then((trx) => {
          return trx.wait();
        });
        setIsLoading(false);
        onDismiss();
