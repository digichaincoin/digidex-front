...
const SortNfts: React.FC<NftSortProps> = ({ nftSet }) => {
  ...
  const sortBy = (sortType) => {
    if (sortType === "rarity") {
      const tempRaritySort = orderBy(filterNftSet, [
    ...
    <>
      <SortHolder>
        <DropDown>
          ...
          <DropDownContent>
            <DropDownItem onClick={() => sortBy("index")}>
              {t("Ape #")}
            </DropDownItem>
            <DropDownItem onClick={() => sortBy("rarity")}>
              {t("Rarity")}
            </DropDownItem>
          </DropDownContent>
        </DropDown>
        <DropDown>
          ...
          <DropDownContent>
            <DropDownItem onClick={() => nextSet()}>
              {t("Next Sale")}
            </DropDownItem>
            <DropDownItem onClick={() => filterBy(1)}>
              {t("Tier 1")}
            </DropDownItem>
            <DropDownItem onClick={() => filterBy(2)}>
              {t("Tier 2")}
            </DropDownItem>
            <DropDownItem onClick={() => filterBy(3)}>
              {t("Tier 3")}
            </DropDownItem>
            <DropDownItem onClick={() => filterBy(4)}>
              {t("Tier 4")}
            </DropDownItem>
            <DropDownItem onClick={() => filterBy(5)}>
              {t("Tier 5")}
            </DropDownItem>
          </DropDownContent>
        </DropDown>
        ...
        <SearchBoxHolder onChange={(e) => search(e)}>
          <SearchBox type="text" placeholder={t("Search by di name")} />
        </SearchBoxHolder>
      </SortHolder>
      <NftList nftSet={nftToDisplay} />
      {endPagination > 50 && (
        <BackToTop onClick={() => backToTop()}>&#x2191;</BackToTop>
      )}
    </>
  );
};
