...
import { Flex, Svg, TagVariants, Text } from "@digi.swap/uikit";
  ...
  return (
    <Flex
      sx={{
        ...
        background: expanded ? "white4" : "white3",
      }}
    >
      <Flex
        sx={{
      ...
      >
        <Flex sx={{ alignItems: "center" }}>
          <Flex>
            ...
            >
              <ServiceTokenDisplay
                token1={CHAIN_PARAMS?.[token?.chainId]?.nativeCurrency?.symbol}
                size={13.5}
              />
            </Flex>
          </Flex>
          ...
          {token?.type &&
            (token?.type === "digiswap" ? (
              <StyledTag variant={"rgb(77, 64, 64)" as TagVariants}>
                {isMobile ? t("DIGI") : t(token?.type.toUpperCase())}
              </StyledTag>
            ) : (
