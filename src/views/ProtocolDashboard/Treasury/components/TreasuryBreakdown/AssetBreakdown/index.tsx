...
const AssetBreakdown: React.FC<{ activeView: number }> = ({ activeView }) => {
  ...
  const sortedOpFunds = filterLowAmounts?.filter(
    (token) => token.location === "Operational Funds"
  );
  const sortedPolFunds = filterLowAmounts?.filter(
    (token) => token.location === "POL"
  );
  const activeAssets = [filterLowAmounts, sortedOpFunds, sortedPolFunds][
    activeView
  ];
  return (
