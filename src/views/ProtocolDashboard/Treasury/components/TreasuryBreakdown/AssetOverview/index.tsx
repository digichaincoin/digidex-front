...
import { Flex, Text } from "@digi.swap/uikit";
...
const setData = (
  assets: (TreasuryAssetOverviewInterface | { symbol: string; value: number })[]
) => {
  return {
...
const sortAndAddAssets = (assets: TreasuryAssetOverviewInterface[]) => {
  ...
  const topSix =
    sortedAssets.length > 6 ? sortedAssets.slice(0, 6) : sortedAssets;
...
const getMergedAssets = (assets: TreasuryAssetOverviewInterface[]) => {
  ...
  const mergedAssets = assets.map((asset: TreasuryAssetOverviewInterface) => {
    const searchAsset = assetTracker.find(
      (searchAsset) => asset.symbol === searchAsset.symbol
    );
    if (searchAsset) {
      const newAsset = {
        symbol: asset.symbol,
...
const AssetOverview: React.FC<{ activeView: number }> = ({ activeView }) => {
  ...
  const treasuryAssets = assets?.filter(
    (asset) => asset.location === "Operational Funds"
  );
  const polAssets = assets?.filter((asset) => asset.location === "POL");
  const mergedAssets = getMergedAssets(assets);

  const cleanedAssets = sortAndAddAssets(
    [mergedAssets, treasuryAssets, polAssets][activeView]
  );
