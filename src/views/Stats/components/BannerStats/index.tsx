...
import { Flex, Link, Skeleton, Svg, Text } from "@digidexfinance/uikit";
...
export const BannerStats = () => {
  ...
  const calculateTotalPortfolio = () => {
    const filteredPortfolio = portfolioData.filter(
      (val) => val.type !== "bills" && val.type !== "iaos"
    );
    ...
    <Container>
      <Header>
        ...
        {isMobile ? (
          ...
          <GridCardsContainer>
            <TranslucidCard>
              <CardLabel>{`DIGICHAIN ${t("Balance")}`}</CardLabel>
                ...
                {!account ? (
                  ...
                  <>
                    <CardValue
                      fontSize="22px"
                      fontWeight={700}
                      value={+stats.userHoldings.digichain}
                      color="primaryBright"
                      decimals={2}
                    />
                    <Svg icon="digichain_token" width={20} />
                  </>
                )}
              </Flex>
            </TranslucidCard>
