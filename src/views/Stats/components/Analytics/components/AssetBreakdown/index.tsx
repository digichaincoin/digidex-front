...
import { BNB, Flex, POLYGON, Text } from "@digidexfinance/uikit";
...
const Asset = React.memo(function Asset({
  amount,
  price,
  symbol,
  balance,
    ...
    <Container>
      <Flex alignItems="center">
        <ServiceTokenDisplay
          token1={
            symbol.includes("-")
              ? wrappedToNative(symbol.split("-")[0])
              : wrappedToNative(symbol)
          }
          token2={
            symbol.includes("-") ? wrappedToNative(symbol.split("-")[1]) : null
          }
