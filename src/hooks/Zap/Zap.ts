...
export function useTradeExactIn(
  ...
  const wrapped =
    (currencyAmountIn?.currency === ETHER &&
      currencyEquals(WETH[chainId], currencyOut)) ||
    (currencyEquals(WETH[chainId], currencyAmountIn?.currency) &&
      currencyOut === ETHER);
  ...
  return useMemo(() => {
    ...
    if (currencyAmountIn && currencyOut && allowedPairs.length > 0) {
      if (singleHopOnly) {
        return (
          Zap.bestZapExactIn(allowedPairs, currencyAmountIn, currencyOut, {
            maxHops: 1,
            maxNumResults: 1,
          })[0] ?? null
        );
...
export function mergeBestZaps(
  ...
  const inAndOutWrappedOne =
    (currencyIn === ETHER &&
      currencyEquals(WETH[chainId], outputCurrencies[0])) ||
    (currencyEquals(WETH[chainId], currencyIn) &&
      outputCurrencies[0] === ETHER);
  const inAndOutWrappedTwo =
    (currencyIn === ETHER &&
      currencyEquals(WETH[chainId], outputCurrencies[1])) ||
    (currencyEquals(WETH[chainId], currencyIn) &&
      outputCurrencies[1] === ETHER);

  // If the input token and output token are the same we need to handle values differently
  const inAndOutAreTheSame1Flag =
    currencyIn === outputCurrencies[0] || inAndOutWrappedOne;
  const inAndOutAreTheSame2Flag =
    currencyIn === outputCurrencies[1] || inAndOutWrappedTwo;
  ...
  } = computeTradePriceBreakdown(chainId, SmartRouter.DIGI, bestZapOne);

  const {
    priceImpactWithoutFee: priceImpactWithoutFeeTwo,
    realizedLPFee: realizedLPFeeTwo,
  } = computeTradePriceBreakdown(chainId, SmartRouter.DIGI, bestZapTwo);
  ...
  const poolTokenPercentage =
    liquidityMinted && totalPairSupply
      ? new Percent(
          liquidityMinted.raw,
          totalPairSupply.add(liquidityMinted).raw
        )
      : null;
