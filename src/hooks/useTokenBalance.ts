...
export const useTotalSupply = () => {
  ...
  const digichainAddress = useDigichainAddress();
  const digichainContract = useERC20(digichainAddress);

  useEffect(() => {
    async function fetchTotalSupply() {
      const supply = await digichainContract.totalSupply();
      setTotalSupply(new BigNumber(supply.toString()));
    }

    fetchTotalSupply();
  }, [slowRefresh, digichainAddress, digichainContract]);
