...
export function useTradeExactIn(
  ...
  bestTradeExactIn = useMemo(() => {
      ...
      if (currencyAmountIn && currencyOut && allowedPairs[index].length > 0) {
        if (singleHopOnly) {
          ...
          for (let i = 1; i <= MAX_HOPS; i++) {
            ...
            if (PRIORITY_SMART_ROUTERS[chainId][0] === SmartRouter.DIGI) {
              if (
                currentTrade?.route?.pairs?.[0]?.router === SmartRouter.DIGI
              ) {
                if (
                  isTradeBetter(
                    bestApeTradeSoFar,
    ...
    if (bestApeTradeSoFar) {
      if (
        parseFloat(bestApeTradeSoFar.priceImpact.toSignificant(6)) <
        DIGI_PRICE_IMPACT
      ) {
