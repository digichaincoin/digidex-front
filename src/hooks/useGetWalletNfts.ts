...
import { useNonFungibleDigis } from "./useContract";
...
const useGetWalletNfts = () => {
  ...
  useEffect(() => {
    const fetchNfts = async () => {
      try {
        const balanceOf = await (
          await nonFungibleDigisContract.balanceOf(account)
        ).toNumber();
  ...
  }, [account, dispatch, nonFungibleDigisContract]);
