import { TokenAmount, Currency, Pair, SmartRouter } from "@digi.swap/sdk";
...
export function useAllSmartPairs(
  currencies: [Currency | undefined, Currency | undefined][]
) {
  const { chainId } = useActiveWeb3React();
    ...
    () =>
      PRIORITY_SMART_ROUTERS[chainId].flatMap((smartRouter: SmartRouter) => {
        return tokens.map(([tokenA, tokenB]) => {
          return tokenA && tokenB && !tokenA.equals(tokenB)
            ? Pair.getAddress(tokenA, tokenB, smartRouter)
            : undefined;
        });
      }),
    [tokens, chainId]
...
export function usePair(
  ...
  return usePairs([[tokenA, tokenB]], smartRouter || SmartRouter.DIGI)[0];
}
