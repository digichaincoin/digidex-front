...
export function useSwapCallArguments(
  ...
  const recipient =
    recipientAddressOrName === null ? activeAccount : recipientAddress;
            ...
            return contract.estimateGas[methodName](...args, options)
              .then((gasEstimate) => {
                return {
                  call,
                  gasEstimate:
                    bestRoute?.routerType === RouterTypes.BONUS
                      ? gasEstimate.mul(BigNumber.from("3"))
                      : gasEstimate,
                };
              })
              .catch((gasError) => {
        ...
        return contract[methodName](...args, {
          ...
          .catch((error: any) => {
            ...
            if (error?.code === 4001) {
              ...
              throw new Error(
                t(`Swap failed: %message%`, { message: error.message })
              );
            }
          });
      },
