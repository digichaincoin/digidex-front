...
export const baseUrl = "https://api.digidex.finance/api/v1";

export const apiBaseUrl =
  process.env.REACT_APP_API_BASE_URL || "https://digi-swap-api.herokuapp.com";
    ...
    const fetchUrl = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.warn(
            "Unable to load signed moonpay url data:",
            response.statusText
          );
        }
      } catch (error) {
...
export const useChartData = (resolution = "60", pair = "DIGICHAIN/BUSD") => {
