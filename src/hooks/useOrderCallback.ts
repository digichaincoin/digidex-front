...
import { Trade, Token } from "@digi.swap/sdk";
  ...
  return useMemo(() => {
    ...
    return {
      ...
      callback: async function onSwap(): Promise<string> {
        ...
        const inputToken =
          input.symbol === "ETH"
            ? getNativeWrappedAddress(chainId)
            : input instanceof Token
            ? input.address
            : "";
        const inputCurrencyDecimals = trade.inputAmount.currency.decimals || 18;
        const inputAmount = parseEther(trade.inputAmount.toSignificant(6)).div(
        ...
        const outputToken =
          output.symbol === "ETH"
            ...
            ? output.address
            : "";
        const outputCurrencyDecimals =
          trade?.outputAmount.currency.decimals || 18;
        const outputAmount = parseEther(outputMinMaxAmount || "0").div(
          10 ** (18 - outputCurrencyDecimals)
        );
