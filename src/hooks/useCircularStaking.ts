...
function useCircularStaking() {
  ...
  if (isEmpty(circStaking)) {
    dispatch(
      setShowModal({ actionType: SHOW_MODAL_TYPES.buyModal, flag: true })
    );
    dispatch(
      setShowModal({
        actionType: SHOW_MODAL_TYPES.generalHarvestModal,
        flag: true,
      })
    );
    dispatch(
      setShowModal({
        actionType: SHOW_MODAL_TYPES.poolHarvestModal,
        flag: true,
      })
    );
    dispatch(
      setShowModal({ actionType: SHOW_MODAL_TYPES.sellModal, flag: true })
    );
  }
}
