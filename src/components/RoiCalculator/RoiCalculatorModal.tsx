...
import { Modal, Button, Text, Tab, Tabs } from "@digi.swap/uikit";
...
const RoiCalculatorModal: React.FC<RoiCalculatorModalProps> = (props) => {
  ...
  const tokensWorthForDollarSelected =
    parseFloat(amountDollars || inputValue) / tokenPrice;
  ...
  const compoundROIRatesValue = Number.isNaN(compoundROIRates)
    ? 0
    : compoundROIRates;
  const percentageCompoundValue = Number.isNaN(parseFloat(percentageCompound))
    ? 0
    : percentageCompound;
  ...
  const rewardPrice = useMemo(() => {
    if (!isLp) {
      const { price } = tokenPrices.find(
        (tok) =>
          tok.address[chainId].toLowerCase() === tokenAddress.toLowerCase()
      );
      return price;
    }
    if (isLp && !lpPrice) {
      const { price } = lpTokenPrices.find(
        (tok) => tok.address[chainId].toLowerCase() === lpAddress.toLowerCase()
      );
      return price;
    }
    return lpPrice;
  ...
  const onTokenAmountChange = useCallback(
    (value: string) => {
      ...
      const fiatValue = parseFloat(
        !!currency && value
          ? (rewardPrice * parseFloat(value))?.toFixed(2)
          : "0"
      );
  ...
  const onDollarAmountChange = useCallback(
    (value: string) => {
      ...
      const expectedValue = parseFloat(
        !!currency && value
          ? (parseFloat(value) / rewardPrice)?.toFixed(6)
          : "0"
      );
      setInputValue(
        Number.isFinite(expectedValue) ? expectedValue.toString() : "0"
      );
    },
    [currency, rewardPrice]
  );
  ...
  return (
    ...
    >
      <Box>
        ...
        <Box sx={styles.tabContainer}>
          <Tabs
            activeTab={compoundIntervals.indexOf(compoundFrequency)}
            variant="fullWidth"
          >
