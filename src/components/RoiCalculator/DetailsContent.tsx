...
import { Button, Svg, useModal } from "@digi.swap/uikit";
...
const DetailsContent: React.FC<DetailsContentProps> = ({
  ...
  const digichain = useDigichainAddress();
  const gdigi = useGoldenDigichainAddress();
  ...
  useEffect(() => {
    if (!isLp) {
      if (tokenAddress?.toLowerCase() === digichain.toLowerCase()) {
        setLink("swap");
      }
      if (tokenAddress?.toLowerCase() === gdigi.toLowerCase()) {
        setLink("gdigi");
      }
    }
  }, [chainId, tokenAddress, isLp, digichain, gdigi]);
    ...
    <>
      ...
      <Box sx={styles.detailContainer(!expanded)}>
          ...
          tokenInfo.map((item) => {
            return (
              <Flex key={item.value} sx={styles.detailRow}>
                ...
                <Text>
                  {item.value === "apr" ? apr.toFixed(2) : apy.toFixed(2)}%
                </Text>
              </Flex>
            );
          })}
        ...
        <Flex sx={{ marginTop: "25px", justifyContent: "center" }}>
          {isLp && !liquidityUrl && (
            <FarmButton
              onClick={() => showLiquidity(tokenAddress, quoteTokenAddress)}
            >
