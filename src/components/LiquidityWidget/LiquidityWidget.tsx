...
import { AddIcon, Button } from "@digi.swap/uikit";
...
const LiquidiyWidget: React.FC<ILiquidityWidgetProps> = ({ onCancel }) => {
  ...
  const [currencyB1, setcurrencyB1] = useState(
    swapCurrencyB || "0x4732A86106064577933552FCea993D30BEC950a5"
  );
  ...
  const balanceA = useCurrencyBalance(
    account ?? undefined,
    currencies[Field.CURRENCY_A]
  );
  const balanceB = useCurrencyBalance(
    account ?? undefined,
  ...
  } ${currencies[Field.CURRENCY_A]?.getSymbol(chainId) ?? ""} and ${
          ...
          <AutoColumn gap="md">
            {(approvalA === ApprovalState.NOT_APPROVED ||
              ...
              isValid && (
                <RowBetween>
                  <Flex
                    sx={{ justifyContent: "space-between", columnGap: "15px" }}
                  >
                    {approvalA !== ApprovalState.APPROVED && (
                      ...
                      >
                        {approvalA === ApprovalState.PENDING ? (
                          <Dots>{`${t("Enabling")} ${currencies[
                            Field.CURRENCY_A
                          ]?.getSymbol(chainId)}`}</Dots>
                        ) : (
                          `${t("Enable")} ${currencies[
                            Field.CURRENCY_A
                          ]?.getSymbol(chainId)}`
