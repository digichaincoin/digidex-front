...
const DropdownDisplay: React.FC<{ inputCurrencies: Currency[]; active? }> = ({
  inputCurrencies,
  active,
}) => {
  ...
  const balance = useCurrencyBalance(
    account ?? undefined,
    pair ? pair?.liquidityToken : inputCurrencies[0]
  );
        ...
        <Text sx={styles.tokenText}>
          {inputCurrencies[1]
            ? `${wrappedToNative(
              ...
              )}-${wrappedToNative(inputCurrencies[1]?.getSymbol(chainId))}`
            : inputCurrencies[0]?.getSymbol(chainId)}
        </Text>
        <Text>
          {!active ? (
