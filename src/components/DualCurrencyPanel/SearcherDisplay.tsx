...
import { Currency, CurrencyAmount } from "@digi.swap/sdk";
...
export function Balance({ balance }: { balance: CurrencyAmount }) {
  return (
    <StyledBalanceText title={balance?.toExact()}>
      {balance?.toSignificant(5)}
    </StyledBalanceText>
  );
}

const SearcherDisplay: React.FC<{
  item: { currencyA: Currency; currencyB: Currency };
}> = ({ item }) => {
  ...
  const balance = useCurrencyBalance(
    account ?? undefined,
    pair ? pair?.liquidityToken : currencyA
  );
          ...
          <Text sx={styles.symbolText}>
            {currencyB
              ? `${wrappedToNative(
                ...
                )}-${wrappedToNative(currencyB?.getSymbol(chainId))}`
              : currencyA?.getSymbol(chainId)}
          </Text>
          <Text size="12px" weight={300} sx={{ lineHeight: "16px" }}>
            {pair
              ? pair?.liquidityToken?.getName(chainId)
              : currencyA?.getName(chainId)}
          </Text>
        </Flex>
        <Text>
          {balance ? (
