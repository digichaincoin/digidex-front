...
import { Text, Flex } from "@digi.swap/uikit";
...
const CTACard: React.FC<CTACardProps> = ({ type, action }) => {
  ...
  return (
    <Flex
      sx={{
        ...circular.ctaCard,
        backgroundImage: [bannersUrl, gdigi && gdigiDesktop],
        backgroundPosition: gdigi && "center",
        color: ((pool || compound) && "brown") || "primaryBright",
        "-webkit-transform":
          (compound || (maximizer && phModal)) && "scaleX(-1)",
      ...
      >
        <Flex
          sx={{
            ...
            "-webkit-transform":
              ((maximizer && phModal) || pool) && "scaleX(-1)",
            textAlign: ((maximizer && phModal) || pool) && "end",
          }}
        >
          <Text sx={circular.ctaTitle}>
            {t(`${content.title.toUpperCase()}`)}
          </Text>
          <Text sx={circular.ctaDescription}>
