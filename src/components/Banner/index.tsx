...
import { Flex, Button } from "@digi.swap/uikit";
...
const Banner: React.FC<{
  ...
  const loaded = useProgressiveImage(
    `../images/new-banners/${banner}-${isDark ? "night" : "night"}.png`
  );

  const openBannerLink = (bannerLink: string) =>
    bannerLink.includes("modal")
      ? history.push({ search: bannerLink })
  ...
  return (
    ...
    >
      {loaded ? (
        <FlexImage
          sx={{ backgroundImage: `url(${loaded})` }}
          maxWidth={maxWidth}
          listViewBreak={listViewBreak}
        />
      ) : (
