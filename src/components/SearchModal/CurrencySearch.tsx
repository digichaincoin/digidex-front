...
import { Currency, ETHER, Token } from "@digi.swap/sdk";
import { Text, Flex, Input } from "@digi.swap/uikit";
...
function CurrencySearch({
  ...
  const showETH: boolean = useMemo(() => {
    ...
    return (
      s.includes("") || s.includes("e") || s.includes("et") || s.includes("eth")
    );
  }, [debouncedQuery]);
  ...
  const getCurrencyListRows = useCallback(() => {
    ...
    return filteredSortedTokens?.length > 0 ||
      (filteredInactiveTokens?.length > 0 && !isZapInput) ? (
      <CurrencyList
        height={335}
        showETH={showETH}
