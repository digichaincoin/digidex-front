...
import { Token } from "@digi.swap/sdk";
...
export function useSortedTokensByQuery(
  ...
  return useMemo(() => {
    ...
    tokens.map((token) => {
      if (token.symbol?.toLowerCase() === "digichain") {
        return digichainToken.push(token);
      }
    ...
    return [...digichainToken, ...exactMatches, ...symbolSubstrings, ...rest];
  }, [tokens, searchQuery]);
}
